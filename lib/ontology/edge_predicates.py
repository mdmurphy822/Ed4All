"""Map concept-graph edge-type slugs to canonical RDF predicates.

Trainforge concept graphs carry edge types as compact slugs in JSON-LD.
The schema-owned slug enum lives in
...
used to lift those slugs to RDF predicates with declared
``rdfs:domain`` / ``rdfs:range`` semantics.

Standard vocabularies are reused when their semantics match. For example:
...
Project-specific predicates are minted in the ``ed4all:`` namespace at
``https://ed4all.dev/ns/courseforge/v1#`` and declared in
``schemas/context/courseforge_v1.vocabulary.ttl`` with full RDFS
semantics, including a label, description, domain, and range.
...
5. Run ``pytest schemas/tests/test_courseforge_vocabulary.py`` — drift
   between the synchronized definitions fails the test suite.
"""
...
# Namespace shared with the canonical Turtle vocabulary.
...
SLUG_TO_IRI: Final[Dict[str, str]] = {
    # Canonical semantic-graph edge types.
    ...
    # Misconception relationships materialize pointers already in the
    ...
    # Pedagogy-graph relation types use their emitted underscore form.
    # ``edge_slug_normalizer.py`` resolves equivalent hyphenated and
    # ``_of``-suffixed forms before lookup so all supported relations map
    # to declared predicates instead of relying on a JSON-LD ``@vocab``.
...
def lookup_iri(slug: str) -> Optional[str]:
    ...
    Normalizer details — including the ``_of`` strip rule and the safety
    guard that prevents fabricating registry entries — live
