...
The OpenStax scan carries vendor MEDIA links as bare / angle-wrapped URLs in
...
def test_linkify_bare_url_html_anchor():
    out = linkify_urls("See (https://openstax.org/l/25Foo) here", html=True)
    assert (
        '<a href="https://openstax.org/l/25Foo" rel="noopener" '
        'class="mathjax_ignore">https://openstax.org/l/25Foo</a>'
    ) in out


def test_linkify_angle_wrapper_dropped():
    out = linkify_urls("X (&lt;https://openstax.org/l/25Bar&gt;) Y", html=True)
    assert "&lt;" not in out and "&gt;" not in out
    assert 'href="https://openstax.org/l/25Bar"' in out
    assert ">https://openstax.org/l/25Bar</a>" in out


def test_linkify_strips_glued_math_delimiters():
    out = linkify_urls("$https://openstax.org/l/25Baz$", html=True)
    assert "$" not in out
    assert 'href="https://openstax.org/l/25Baz"' in out


def test_linkify_normalizes_ocr_spaced_scheme():
    out = linkify_urls("go https : //openstax.org/l/25Q end", html=False)
    assert out == "go https://openstax.org/l/25Q end"


def test_linkify_preserves_trailing_sentence_period():
    out = linkify_urls("end https://openstax.org/l/25Z. Next.", html=True)
    assert ">https://openstax.org/l/25Z</a>. Next." in out


def test_linkify_plain_mode_no_tag():
    out = linkify_urls("(https://openstax.org/l/25P)", html=False)
    assert out == "(https://openstax.org/l/25P)"
    assert "<a" not in out


def test_linkify_idempotent():
    once = linkify_urls("(https://openstax.org/l/25P) x", html=True)
...
def test_adapter_linkifies_block_url_with_mathjax_ignore():
    ...
    assert 'href="https://openstax.org/l/25ProductProp"' in html
    ...
    assert "https://openstax.org/l/25ProductProp" in sec_text
