...
Regression for the 2026-08-01 vendor-corpus finding: ``figure_placeholder``
...
def test_visible_body_is_never_the_bare_figure_token():
    for alt in (TEX, PROSE, "a", "."):
        assert _body(figure_placeholder(alt, html=True)) != "[figure]"


def test_tex_alt_is_delimited_for_mathjax():
    body = _body(figure_placeholder(TEX, html=True))
...
def test_prose_alt_is_plain_visible_text():
    body = _body(figure_placeholder(PROSE, html=True))
    assert body == PROSE
...
def test_aria_label_contract_is_unchanged():
    ...
    for alt in (TEX, PROSE):
        assert _label(figure_placeholder(alt, html=True)) == (
            f"{alt} (image not recoverable)"
        )


def test_plain_mode_unchanged():
    assert figure_placeholder(TEX, html=False) == f"[figure: {TEX}]"


def test_empty_alt_still_degrades_to_figure():
    span = figure_placeholder("", html=True)
...
def test_body_is_html_escaped():
    span = figure_placeholder('<b>&"x"', html=True)
