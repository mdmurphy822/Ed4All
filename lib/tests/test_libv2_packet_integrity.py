...
def test_complete_synthetic_archive_has_zero_critical(tmp_path: Path):
    """A complete neutral archive exercises all rules without operator data."""
    archive = _baseline_archive(tmp_path)
    result = PacketIntegrityValidator().validate(archive)
    assert (
        result.critical_count == 0
    ), (
        f"Expected 0 critical issues on synthetic baseline, got "
