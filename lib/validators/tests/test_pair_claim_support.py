"""Wave W-D1 T1.4 — `PairClaimSupportValidator.validate()` walk tests.

Audit-walk only: asserts every pair on disk carries `per_claim_support`
+ `claim_support_rate` keys (values can be None — deps-missing arm).
Per the validator docstring at `pair_claim_support.py:1202-1228`, this
is NOT a re-run of the per-pair NLI fan-out; that path is exercised by
`Trainforge/tests/test_pair_claim_support.py`.
...
def test_pairs_with_audit_fields_pass(tmp_path: Path) -> None:
    ...
    assert event["decision_type"] == "pair_claim_support_check"
...
def test_pair_missing_per_claim_support_fires_critical(tmp_path: Path) -> None:
    capture = _StubCapture()
    inst = tmp_path / "instruction_pairs.jsonl"
    ...
    assert result.passed is False
    codes = [i.code for i in result.issues]
    assert "MISSING_PER_CLAIM_SUPPORT" in codes
    assert any(i.severity == "critical" for i in result.issues)
    assert capture.calls[0]["decision"].startswith("audit:failed")


def test_pair_missing_claim_support_rate_fires_critical(tmp_path: Path) -> None:
    capture = _StubCapture()
    inst = tmp_path / "instruction_pairs.jsonl"
    pair = _pair_with_audit_fields()
    pair.pop("claim_support_rate")
    _write_jsonl(inst, [pair])
    ...
    assert result.passed is False
    codes = [i.code for i in result.issues]
    assert "MISSING_CLAIM_SUPPORT_RATE" in codes
