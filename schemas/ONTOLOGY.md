...
- **`schemas/models/model_card.schema.json`** (Wave 89, extended Wave 92). Draft 2020-12, strict additionalProperties at every level. Required: `model_id` (kebab-case), `course_slug`, `base_model{name,revision,huggingface_repo}`, `adapter_format` (enum: `safetensors` / `gguf` / `merged_safetensors`), `training_config` (seed, learning_rate, epochs, lora_rank, lora_alpha, max_seq_length, batch_size), `provenance` (seven SHA-256 hashes — see below), `created_at`. Optional: `eval_scores` (sub-object dropped in directly from `Trainforge/eval/slm_eval_harness.py` output), `license`, `description`, `tags`. Wave 92 added `holdout_graph_hash` to `provenance.required` + `provenance.properties` so Bloom-stratified KG holdout splits are pinned alongside the chunks/graphs.
...
- **7-hash provenance pattern — the canonical SLM-archival contract.** `model_card.json::provenance` pins one run to seven LibV2 paths: `chunks_hash` (`corpus/chunks.jsonl`), `pedagogy_graph_hash` (`graph/pedagogy_graph.json`), `instruction_pairs_hash` + `preference_pairs_hash` (`training_specs/*.jsonl`), `concept_graph_hash` (`graph/concept_graph_semantic.json`), `vocabulary_ttl_hash` (`graph/vocabulary.ttl`), `holdout_graph_hash` (Wave 92 KG holdout). Hash mismatch on import fails closed via `LibV2ModelValidator`. This is what makes adapter retraining on a new base model auditable end-to-end without re-chunking.
- **`CourseManifest.slm_processing` (now written, Wave 93).** The slot has lived on `CourseManifest` since the early waves but was unused. Wave 93's `LibV2/tools/libv2/importer.py::import_model` populates it from `model_card.json` on every adapter import — `slm_version`, `generation`, `parent_version` now flow through to `manifest.json` and are surfaced by `libv2 models list <slug>`. Retrieval consumers can fast-skip courses without a trained adapter by inspecting this field.
- **Decision events.** `decision_event.schema.json::decision_type` enum gains 6 SLM-pipeline values (alphabetised insertion preserved): `base_model_selection`, `eval_run_decision`, `hyperparameter_selection`, `model_promotion_decision`, `synthesis_provider_call` (Wave 91), `training_run_planning`. Phase enum gains `trainforge-training`. Without these, `DECISION_VALIDATION_STRICT=true` would fail-close on Wave 90+ training runs.
- **Validators.** `lib/validators/libv2_model.py::LibV2ModelValidator` (Wave 89) is the gate for both `import_model` (writes) and the `libv2_model` schema-match path. Wave 91 adds `lib/validators/min_edge_count.py` + `lib/validators/synthesis_diversity.py` as pre/post synthesis gates wired into `config/workflows.yaml::textbook_to_course::training_synthesis`, and promotes `kg_quality_report` thresholds off advisory 0.0 to 0.95 / 0.95 / 0.95 / 0.5.
