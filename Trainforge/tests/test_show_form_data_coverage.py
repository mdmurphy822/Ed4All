...
from Trainforge.scripts.ops import show_form_data_coverage as cli  # noqa: E402
...
def test_show_latest_table_format(tmp_path: Path) -> None:
    ...
    with redirect_stdout(out):
        rc = cli.main([
            "--course-code", "rdf-shacl-synthetic",
...
def test_show_falls_back_to_live_snapshot_when_default_checkpoint_absent() -> None:
    ...
    with redirect_stdout(out), redirect_stderr(err):
        rc = cli.main(["--course-code", "rdf-shacl-synthetic"])
    assert rc == 0, f"expected live fallback, got rc={rc}, stderr={err.getvalue()!r}"
    out_text = out.getvalue()
    assert "live snapshot" in out_text.lower()
    assert "rdf-shacl-synthetic" in out_text
    # Family map should render — the rdf-shacl-synthetic family_map exists in repo.
...
def test_show_live_fallback_emits_same_shape_in_json_format() -> None:
    ...
    out = io.StringIO()
    with redirect_stdout(out):
        rc = cli.main(["--course-code", "rdf-shacl-synthetic", "--format", "json"])
    assert rc == 0
    payload = json.loads(out.getvalue())
    assert payload["live"] is True
    assert payload["course_slug"] == "rdf-shacl-synthetic"
