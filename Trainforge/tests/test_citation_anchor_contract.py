...
The extraction-contract gate and per-kind floors are exercised against
temporary copies of the neutral tracked mini-course. Operator archives are
never discovered or read.
...
# ---------------------------------------------------------------------------
# Current-contract floor enforcement on a neutral temporary corpus
# ---------------------------------------------------------------------------
#
_PER_KIND_FLOOR = {
    "dart": 0.96,
...
@pytest.mark.parametrize("kind", ["dart", "imscc"])
def test_current_contract_corpus_enforces_anchoring_floor(tmp_path, kind):
    course_dir = tmp_path / "course"
    shutil.copytree(MINI_COURSE, course_dir)
    chunks_path = course_dir / "semantik_chunks" / "chunks.jsonl"
    _write_json(course_dir / "course_manifest.json", {
        "chunker_version": "v4",
        "extraction_contract": EXTRACTION_TEXT_CONTRACT_VERSION,
    })
    ...
    assert extraction_contract == EXTRACTION_TEXT_CONTRACT_VERSION
    assert chunker_version == "v4"
    report = anchor_report(chunks_path, course_dir, chunkset_kind=kind)
    rate = report["anchoring_rate"]
    assert rate >= _PER_KIND_FLOOR[kind], report["status_counts"]


def test_neutral_imscc_fixture_keeps_source_archive():
    assert list((MINI_COURSE / "source" / "imscc").glob("*.imscc"))
