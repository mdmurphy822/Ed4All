...
| `TRAINFORGE_CHUNK_TYPE_CONTENT_AWARE` | When on, `Trainforge/chunker/chunker.py::_type_from_heading` swaps the legacy pure-substring chunk-type matcher for the whole-word / course-title-aware classifier `_type_from_heading_content_aware`. The substring matcher mis-types on incidental words (class names like `DocumentSummaryBase` → `summary`, bare `NVIDIA Course NN:` page titles keying off an incidental word, `[Exercise] ... Example` headings losing to `example`); the content-aware path matches whole words and falls bare course/notebook page-title headings through to the neutral `explanation` default. Fixes the ~7% chunk-type misclassification audited on the NVIDIA corpus. Part of the operator's known-good chunking config. Default off → the RDF/SHACL calibration corpus stays byte-identical on rebuild. **Marketable-v1 A5: auto-on for `textbook_to_course` / `course_generation` runs** via `_apply_corpus_generalization_defaults` (setdefault); bare library calls keep default-off. |
