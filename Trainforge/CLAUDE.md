...
The canonical chunker lives at `Trainforge/chunker/` and exposes `chunk_content`, `chunk_text_block`, `merge_small_sections`, `merge_section_source_ids`, plus the `ChunkerContext` dataclass. It is shared by DART (Phase 7b `_run_dart_chunking`), the IMSCC path (Phase 7c), and Trainforge synthesis — one canonical chunker, one source of truth. Phase 7a Subtask 6 (commit `64d5e3e`) originally lifted these helpers out of `Trainforge/process_course.py` into a standalone `ed4all-chunker` workspace package; the post-Phase-8 review re-merged the package back into `Trainforge/chunker/` to remove the workspace-member friction (see `plans/post-phase8-review-2026-05.md`). Public names and the `ChunkerContext` callback contract are unchanged — only the import path moved (`ed4all_chunker.X` → `Trainforge.chunker.X`).

`Trainforge/process_course.py::CourseProcessor` retains the four method wrappers (`_chunk_content`, `_chunk_text_block`, `_merge_small_sections`, `_merge_section_source_ids`) for back-compat with direct callers (a handful of test sites import `CourseProcessor` directly). Each wrapper builds a `ChunkerContext` that threads `_create_chunk` — the only chunker helper that stays on `CourseProcessor`, because of its deep coupling to instance state (`run_id`, decision-capture handle, source-reference accumulator) — back into the chunker via callback. `chunk_content` returns a `ChunkContentResult` dataclass `(chunks, pages_with_misconceptions)` so the wrapper can apply the `pages_with_misconceptions` side effect on `self` after the call returns. Post-re-merge these are sibling-module calls, not cross-package delegation.

Phase 7a Subtask 8 added a top-level `chunker_version` field to `course_manifest.json`, sourced from `Trainforge.chunker.CHUNKER_SCHEMA_VERSION` (currently `"v4"`), so LibV2 audits can pin which chunker shipped each corpus. See root `CLAUDE.md` § "Project Structure" for the canonical location.
...
| 137 followup | `7a85f34` | `align_chunks.py` honest provenance + per-chunk teaching-role checkpoint — `teaching_role_source` is now owned by the classifier helpers themselves (`"llm"` on success, `"llm_failed"` + `teaching_role_failure` dict on exception); pre-fix, a blanket caller-level `setdefault("llm")` mislabeled 26 chunks whose LLM call had silently failed. New `_load_teaching_role_checkpoint` / `_append_teaching_role_checkpoint` helpers + default sidecar at `<corpus>/corpus/.teaching_role_checkpoint.jsonl`; resume replays cached `teaching_role` + source + failure metadata, sidecar unlinked on clean exit. 13 regression tests across `test_align_chunks_provenance.py` + `test_align_chunks_checkpoint.py`. |
| 138a / W1 | `65a0b8f` | `Trainforge/eval/teaching_role_alignment.py::TeachingRoleAlignmentEvaluator` — Tier-2 corpus check that aggregates `teaching_role` distribution per `content_type_label`, compares against `DEFAULT_EXPECTED_MODES` (7 content types), and emits `content_type_role_alignment` + `content_type_role_alignment_summary` with per-content-type entries and an `alignment_rate`. Pure declarative (zero LLM calls); wall-time 8.9 ms on the rdf-shacl-551-2 corpus. `DEFAULT_MIN_CHUNKS_FOR_FLAG=5` skips below-threshold buckets. 9 regression tests. |
| 138a / W2 + WorkerC | `9c72791` | `slm_eval_harness.py` wires the W1 evaluator via `_run_stage("teaching_role_alignment", ...)` before behavioral invariants and folds output into `eval_report.json` at top level for the W3 validator. Same commit lands per-stage eval checkpoint sidecar at `<adapter>/eval/.eval_results_checkpoint.jsonl` (tolerant JSONL loader, schema-version drop, append + flush per stage, unlink on clean exit, preserve on exception); CLI `--no-eval-checkpoint` opt-out. 18 regression tests in `test_eval_harness_integration.py` (3 W2 + 6 WorkerC). |
| 138a / W3 | `6c18869` | `EvalGatingValidator` adds the `min_content_type_role_alignment_rate` threshold (default 0.70, warning-severity, code `EVAL_CONTENT_TYPE_ROLE_ALIGNMENT_LOW`); rationale interpolates `alignment_rate` + `mismatched_content_types`. Skips silently on legacy reports lacking the field. `model_card.schema.json::eval_scores` extended with permissive `content_type_role_alignment` + `content_type_role_alignment_summary` objects. 3 new regression tests (13/13 EvalGatingValidator green). |
| 138a / WorkerA | `6baf6c0` | Per-pair resume checkpoint sidecar in `synthesize_training.py` at `training_specs/.synthesis_pairs_checkpoint.jsonl`. One JSONL line per accepted instruction / preference pair (post-validation, post-decoration), flushed per emit. `_load_synthesis_pairs_checkpoint` rebuilds a `(chunk_id, kind, variant_index)` cache; the chunk loop short-circuits past every cached key. Preserved on `SynthesisBudgetExceeded` + any other exception, unlinked on clean exit. Provider mismatch warns + discards; schema-version drift logs + skips. CLI `--no-checkpoint` opt-out. 7 regression tests mirroring the `align_chunks.py` reference pattern. Closes the gap that Wave 116's `.in_progress` sidecar (observability-only, opens in `"w"`) and Wave 107's `.synthesis_cache.jsonl` (claude_session-only) left for `local` / `together` / `anthropic` providers. |
| 138a / WorkerB | `915cbc4` | Per-session resume log for the FORM_DATA backfill loop — `Trainforge/scripts/backfill_form_data.py` writes per-CURIE outcomes to `LibV2/courses/<slug>/.backfill_session.jsonl` (or next-to-catalog-YAML fallback). `max_redrafts_exceeded` CURIEs are filtered out of subsequent sessions unless `--retry-failed` is passed; `--clean-session` resets the log; the file is unlinked when the family is fully complete. 4 new regression tests (per-CURIE append, prior-failure skip, retry-failed flag, clean-session unlink). |
| 138b / W5 | _this commit_ | Heuristic-extension fix closing the Wave 138a `scenario` / `transfer` gap. `Trainforge/align_chunks.py::_heuristic_role` extended with `content_type_label`-aware rules: `("real_world_scenario", "scenario") -> "transfer"`, `("self_check", "assessment", "assessment_item") -> "assess"`, `"summary" -> "synthesize"`. The new branch fires AFTER the legacy `chunk_type=assessment_item` check (so assessment items still win) but BEFORE the legacy `resource_type` checks (so chunker-emitted content_type_label signal beats container heuristics). The 4-role LLM curriculum-alignment enum cannot return `transfer` or `assess` — without this branch, scenario chunks fall through and never get `transfer`. Each new-rule fire emits a `decision_type="teaching_role_heuristic_extended"` decision event interpolating chunk_id / role / triggering label into the rationale; new enum value registered in `schemas/events/decision_event.schema.json`. Q2 (`procedure -> elaborate@0.70`) added to `Trainforge/eval/teaching_role_alignment.py::DEFAULT_EXPECTED_MODES` per the plan's operator-recommended answers; Q3 (`example`) intentionally left out (corpus distribution is bimodal between `elaborate` and `reinforce`, no confident expected mode). Synthetic re-run on rdf-shacl-551-2: `scenario` flips from 0/8 to 8/8 transfer; `alignment_rate` rises from 0.80 to 0.833 (procedure becomes a new mismatch at 0.61 actual vs 0.70 floor — surfaces drift via the warning-severity gate but doesn't block, per Q5). 13 new tests in `Trainforge/tests/test_align_chunks_heuristic_wave138b.py` + 1 new test in `test_teaching_role_alignment.py` covering the Q2 default. |
...
- **Checkpoint-aware resume across all providers** (Wave 138a / WorkerA) — `Trainforge/synthesize_training.py::run_synthesis` appends every accepted instruction / preference pair to `training_specs/.synthesis_pairs_checkpoint.jsonl` (post-validation, post-decoration, flushed per emit). On restart, `_load_synthesis_pairs_checkpoint` rebuilds a `(chunk_id, kind, variant_index)` cache and the chunk loop short-circuits past every cached key without dispatching to the LLM. Preserved on `SynthesisBudgetExceeded` + any exception; unlinked on clean exit. Provider mismatch + schema-version drift invalidate stale records loudly. Closes the resume gap for `local` / `together` / `anthropic` providers — Wave 116's `.in_progress` sidecar opens in `"w"` (observability only, truncates on restart) and Wave 107's `.synthesis_cache.jsonl` is wired only for `claude_session`. CLI `--no-checkpoint` opt-out for callers who want the legacy from-scratch behavior.
