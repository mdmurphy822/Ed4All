...
The directory name is derived from the active `DecisionCapture.phase`, which must be one of the canonical hyphenated values in `schemas/events/decision_event.schema.json`. `CourseProcessor` uses `phase="trainforge-content-analysis"` (see `Trainforge/pipeline/process_course.py`).
...
| CLI `python -m Trainforge.synthesis.synthesize_training` | Direct module invocation. | The full `run_synthesis` surface, including `--synthesis-contract`, `--include-dpo-from-misconceptions`, `--with-assessment-sft`, `--with-graph-sft`. |
...
Training is a **post-import LibV2 stage**, not a step in `Trainforge/pipeline/process_course.py`. Consequences:
...
- **No empty `decision_capture_id`** — `Trainforge/synthesis/synthesize_training.py::_last_event_id` raises `RuntimeError` when called against a capture with zero logged events. Every synthesis emit assigns the ID after at least one decision is logged, so any path that would have written an empty string is now a hard failure.
...
- **Empty-field misconception drops are audit-logged** — `Trainforge/synthesis/synthesize_training.py::_build_misconception_dpo_pair` logs a `decision_type="misconception_pair_skipped"` event before returning `None` on an empty `misconception` / `correction` field. The enum entry was added to `schemas/events/decision_event.schema.json` so `DECISION_VALIDATION_STRICT=true` does not drop the audit event.
...
`Trainforge/synthesis/synthesize_training.py` gates the chunk → instruction-pair paraphrase pass behind a provider parameter. **Licensing posture per provider:** see `docs/LICENSING.md` for the canonical reference (ToS layer + underlying model license per row, citation URLs, decision tree). The summary below is operational.
...
**Instruction variants.** `--instruction-variants-per-chunk` (default `1`) controls how many SFT instruction pairs each eligible chunk emits. The frames live in `Trainforge/synthesis/synthesize_training.py::_INSTRUCTION_PROMPT_FRAMES`:
...
This surface exports **decision-capture** data keyed by run. It is distinct from the SLM training corpus in `LibV2/courses/<slug>/training_specs/` (`instruction_pairs.jsonl` / `preference_pairs.jsonl`), which is produced by `Trainforge/synthesis/synthesize_training.py` and consumed directly by `Trainforge.cli.train_course` — see § Provider configuration (synthesis) and § Training Pipeline.
