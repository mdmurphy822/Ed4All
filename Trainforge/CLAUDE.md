...
| `local` (Wave 113) | Real-LLM paraphrase via `Trainforge/generators/_local_provider.py` against any OpenAI-compatible local server (Ollama / vLLM / llama.cpp / LM Studio). Default base URL `http://localhost:11434/v1`; default model `qwen2.5:14b-instruct-q4_K_M`. Overrides via `LOCAL_SYNTHESIS_BASE_URL` / `LOCAL_SYNTHESIS_MODEL`. | A running local model server. `LOCAL_SYNTHESIS_API_KEY` optional. | Outputs licensed per `LOCAL_SYNTHESIS_MODEL`. Default `qwen2.5:14b-instruct-q4_K_M` is Apache 2.0 (training-permitted, no attribution required for outputs). **Recommended default for license-clean training-data synthesis.** See `docs/LICENSING.md`. |
