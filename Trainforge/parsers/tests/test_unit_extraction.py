"""Wave #22 Tier-2 — the parser harvests the composite-unit type off the unit
wrapper onto the unit's LEAD section, and the ``<section class="semantik-unit">``
wrapper is transparent to text extraction (no attribute/markup leak).

The parser dual-reads both the current ``data-semantik-*`` spelling and the
legacy ``data-dart-*`` spelling; the harvested value lands on
``ContentSection.data_dart_unit``. Synthetic HTML strings only (no corpus
files).
"""
...
# A worked-example composite unit: the <section class="semantik-unit"> wrapper
# folds the Example callout-group + the Solution callout-group. Only the LEAD
# block (Example 9.1, the wrapper's first child) is the unit EDGE.
_UNIT_HTML = """
...
<div class="semantik-callout-group" data-semantik-opener-group="worked_example">
<section class="semantik-section" aria-labelledby="ex-9-1" data-semantik-block-id="ex-9-1"
         data-semantik-source="synthesized" data-semantik-opener="worked_example">
<h4 id="ex-9-1">Example 9.1</h4>
</section>
<section class="semantik-section" id="p1" data-semantik-block-id="p1" data-semantik-source="synthesized">
<p>Simplify the radical expression fully.</p>
</section>
</div>
<div class="semantik-callout-group" data-semantik-opener-group="solution">
<section class="semantik-section" aria-labelledby="sol-1" data-semantik-block-id="sol-1"
         data-semantik-source="synthesized" data-semantik-opener="solution">
...
def test_unit_wrapper_transparent_to_text_extraction():
    ...
    assert "semantik-unit" not in text
...
def test_legacy_data_dart_unit_still_harvested():
    # Legacy-compat read path for the pre-purge attribute spelling.
    by_heading = _sections_by_heading(_LEGACY_UNIT_HTML)
    assert by_heading["Example 9.1"].data_dart_unit == "worked_example"
