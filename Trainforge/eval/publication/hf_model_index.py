"""Hugging Face model-index converter for evaluation reports.

Converts the ``eval_report.json`` shape produced by
...
def write_hf_readme(
    ...
    *,
    ...
    Args:
        ...
        eval_report: Eval-report shape (see
        ...
        model_card: The model_card.json dict; we read
        ...
        course_path: Path to the LibV2 course directory
    ...
    tags = [
        ...
        # Branding tags route leaderboard searches to this adapter.
        "ed4all-bench", "grounded-slm-benchmark",
    ]
    # Prefer manifest-declared classification tags over substring inference.
    # This lets
    ...
    # courses without classification metadata retain compatible behavior.
    ...
    if manifest_tags:
        ...
        # Infer tags when classification metadata is unavailable.
    ...
    # Lead with the headline sentence so readers see the procurement claim
...
def _extract_pass_rate(payload: Any) -> Optional[float]:
    ...
    Invariant evaluators emit dictionaries like::
...
def _render_body(
    ...
    lines.append("")
    # Open with the thesis statement so the procurement claim is visible to
    # readers who skim the generated README.
    lines.append(
    ...
    # Surface hallucination_rate directly instead of requiring readers to
    # reconstruct it from faithfulness.
    ...
    # Render headline and retrieval-method ablation tables together.
    if ablation_report:
        lines.extend(_render_headline_table(ablation_report))
        lines.extend(_render_retrieval_method_table(ablation_report))

    # Keep qualitative diagnostic findings beside the metric tables.
    ...
    # Identify the synthesis input whose variation contributes to eval drift.
    ...
    # Publish the commands and hashes needed to reproduce the evaluation.
