...
# Add project path
PROJECT_ROOT = Path(__file__).resolve().parents[3]  # → Ed4All/
...
class QuestionData:
    ...
    # Observed Bloom level supplied by the configured classification backend
    # plus its alignment signal. Round-tripped unconditionally
...
class AssessmentGenerator:
    ...
    def _apparatus_guard(self, result):
        ...
        placeholder.
        """
        from Trainforge.generators.assessment.content_extractor import _is_apparatus_text
    ...
    def generate(
        ...
        if ensure_objective_coverage:
            ...
            # Preserve objective coverage after rejecting contaminated or
            # otherwise unusable source material. Retry only uncovered
            # objectives against the full source pool, rotating Bloom levels
            # so the extractor advances to different grounded material.
        ...
        if self.capture and skipped_items:
            ...
            self.capture.log_decision(
                ...
                rationale=(
                    ...
                    f"placeholder emission is prohibited because strings "
                    f"such as 'Correct answer based on content' poison the "
            ...
            if self.capture:
                self.capture.log_decision(
                    ...
                    rationale=(
                        ...
                        f"Deterministic placeholder emission is prohibited; "
                        f"strings such as 'Correct answer based on content' "
                        f"would contaminate the "
    ...
    def _generate_multiple_choice(
        ...
        if source_chunks:
            ...
            if terms:
                ...
                # Rotate the statement pool so each question does not reuse the
                # same leading statements as distractors. Reuse the shared
                        ...
                        try:
                            self.capture.log_decision(
                                ...
                                decision=(
                                    ...
                                    f"resolved (need 3); padded distractors "
                                    f"are prohibited."
                                ),
                                rationale=(
                ...
                return QuestionData(
                    ...
                    generation_rationale=(
                        ...
                        f"content statements verified by the negation check"
                    ),
                )
                ...
                if make_false:
                    return QuestionData(
                        ...
                        generation_rationale=(
                            ...
                            f"(verified by the negation check)"
                        ),
                    )
                else:
                    return QuestionData(
    ...
    def generate_diversified(
        ...
        def _emit(result, subtype: str) -> None:
            ...
            any_q = False
            for it in items:
                # Apply the apparatus guard at this tier's single assembly seam
                # so every diversified subtype, including future builders,
                # receives the same protection by construction.
        ...
        if ensure_objective_coverage:
            ...
            # Repair builder/fallback misses before count-fill. A
