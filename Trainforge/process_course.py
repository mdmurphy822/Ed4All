...
# chunk shape (ADR-001 Contract 1). v4 adds `source.html_xpath` and
# `source.char_span` for audit-trail provenance (Worker E), and is shared
# with Worker D's per-chunk summary + retrieval_text fields. The version
# string lands on manifest.json as `chunk_schema_version` and on every
# chunk as `schema_version`.
