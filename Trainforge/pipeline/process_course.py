...
Usage:
    python -m Trainforge.pipeline.process_course \
        --imscc <IMSCC_PATH> \
...
def _resolve_target_models() -> List[str]:
    ...
    Priority order:
      ...
      2. ``TARGET_MODELS_DEFAULT`` (stable default output shape).
...
def _generate_chunk_id(prefix: str, start_id: int, text: str, source_locator: str) -> str:
    ...
    Default: position-based ``f"{prefix}{start_id:05d}"``.
...
def _route_misconception_to_tag(
    ...
    the result is deterministic). Returns the first tag in ``tags``
...
class CourseProcessor:
    ...
    def __init__(
        ...
        self.corpus_dir = self.imscc_chunks_dir  # supported read alias
    ...
    def process(self) -> Dict[str, Any]:
        ...
        # unreadable / corrupted upstream chunks so callers without an
        # upstream chunkset still use the in-process build.
    ...
    def _chunk_content(self, parsed_items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        - Emits the stable ``"  Generated N chunks"`` operator summary.
        """
    ...
    def _create_chunk(
        ...
        # default OFF keeps the standard output byte-identical). Matches the inline
        ...
        # Thread an aggregate source-document SHA into every chunk's source block when the
        ...
        # authoritative shape is missing, the field is
        ...
        # None when no match. Optional field; skipped on no-match so chunks
        # do not grow a null cognitive_task_type slot.
        ...
        # means absent. Chunks without source audit arrays omit these optional fields rather
        ...
        # 8601 UTC. Both fields are optional at the schema level, so chunks
        # without them continue to validate.
        #
    ...
    def _extract_section_metadata(
        ...
        *,
        ...
        preserving the absence contract for chunks without these keys.
        ...
        # to the established single-section behavior), then merged headings not already in
        # the list.
    ...
    def _build_learning_outcome_source_refs(
        ...
        dict so an absent reverse map remains omitted from output.
        """
        if not lo_refs:
            return {}
        cf_meta = item.get("courseforge_metadata") or {}
        ...
        for entry in lo_entries:
            ...
            if collected:
                # Multiple JSON-LD entries may share the same id. Extend rather than overwrite so no
    ...
    def _build_misconceptions_for_graph(
        ...
        Concept-routing precedence:
          ...
          3. First concept tag fallback.

        The token-overlap path handles authored misconceptions without an
        explicit ``concept_id``. In that case a first-tag-only heuristic
    ...
    def _generate_semantic_concept_graph(
        ...
        through to the in-process ``build_semantic_graph`` build so compatible
        corpora and stale phase-output handoffs remain processable.
        """
    ...
    def _build_objectives_metadata_for_graph(
        ...
        (including non-Courseforge IMSCCs) we reconstruct the shape
    ...
    def _build_pedagogy_summary(
        ...
        if pedagogy_graph is not None:
            ...
            # concept slugs flat to match the chunk-based path.
    ...
    def _build_course_json(self, manifest: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            schema_path = PROJECT_ROOT / "schemas" / "knowledge" / "course.schema.json"
        ...
        for ch in self.objectives.get("chapter_objectives", []):
            ...
            for obj in inner:
                ...
                if obj.get("source_refs"):
                    ...
                    # which is correct for List[str] but not for
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--llm-provider", default="mock", choices=["mock", "anthropic", "together", "local"],
                   help=(
                       "LLM provider for the --align direct-classification path "
    ...
    p.add_argument(
        ...
        help=(
            ...
            "absent / unreadable (compatible corpora remain unchanged)."
        ),
    )
    p.add_argument(
        "--imscc-chunks-path",
        ...
        help=(
            ...
            "when absent / unreadable so callers without a pre-built "
            "chunkset remain supported."
        ),
    )
    return p
