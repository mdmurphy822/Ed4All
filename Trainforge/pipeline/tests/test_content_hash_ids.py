"""Verify position-based and opt-in content-hash chunk identifiers.

Regression tests for the ``_generate_chunk_id`` helper in
``Trainforge.pipeline.process_course`` and for the relaxed ``chunk_v4.schema.json``
``id`` pattern.
...
# Ed4All/Trainforge/pipeline/tests/test_content_hash_ids.py → parents[3].
PROJECT_ROOT = Path(__file__).resolve().parents[3]
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from Trainforge.pipeline.process_course import _generate_chunk_id  # noqa: E402
...
def test_flag_off_uses_position(monkeypatch):
    ...
    chunk_id = _generate_chunk_id(
        prefix="fixture_201_chunk_",
        start_id=42,
        text="some text",
        source_locator="course_content/page_01.html",
    )
    assert chunk_id == "fixture_201_chunk_00042"
    assert re.match(r"^fixture_201_chunk_\d{5}$", chunk_id)
...
def test_flag_off_explicit_false(monkeypatch):
    ...
    for value in ("", "false", "False", "0", "no"):
        ...
        chunk_id = _generate_chunk_id(
            prefix="fixture_201_chunk_", start_id=7, text="t", source_locator="p",
        )
        assert chunk_id == "fixture_201_chunk_00007", (
...
def test_flag_on_uses_content_hash(monkeypatch):
    ...
    assert re.match(r"^fixture_201_chunk_[0-9a-f]{16}$", chunk_id), chunk_id
...
def test_content_hash_stable_across_runs(monkeypatch):
    ...
    a = _generate_chunk_id(
        prefix="fixture_201_chunk_", start_id=0,
        text="stable content for hashing", source_locator="path/a.html",
    )
    b = _generate_chunk_id(
        prefix="fixture_201_chunk_", start_id=999,  # start_id must not affect hash
        text="stable content for hashing", source_locator="path/a.html",
    )
...
def test_schema_accepts_both_formats():
    ...
    assert regex.match("fixture_201_chunk_00001")
    assert regex.match("fixture_201_chunk_99999")

    # New content-hash form — must pass.
    assert regex.match("fixture_201_chunk_a3f2b9c8d1e4f567")
    assert regex.match("fixture_201_chunk_0123456789abcdef")

    # Malformed suffixes — must NOT match (negative controls).
    assert not regex.match("fixture_201_chunk_123")  # too few digits
    assert not regex.match("fixture_201_chunk_invalid")  # non-hex letters
    assert not regex.match("fixture_201_chunk_a3f2b9c8d1e4f5")  # 14 hex chars
    assert not regex.match("fixture_201_chunk_A3F2B9C8D1E4F567")  # uppercase hex
...
def test_full_schema_validates_legacy_and_hash_ids():
    ...
    # Legacy form
    legacy = _make_valid_chunk("fixture_201_chunk_00001")
    ...
    # Content-hash form
    hashed = _make_valid_chunk("fixture_201_chunk_a3f2b9c8d1e4f567")
