...
# even-page leading-digit footer pattern (``"{N} A.W. (Tony) sample-corpus"``).
...
def _strip_leading_digits(normalised: str) -> Tuple[str, Optional[int]]:
    ...
    ``"{N} A.W. (Tony) sample-corpus"`` put the page number BEFORE the fixed
...
def detect_page_chrome(
    ...
    *,
    ...
    # digit (``"42 A.W. sample-corpus"``). The frequency counter then evaluates
    ...
    def _derive_keys(raw_line: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        leading integer (e.g. ``"a.w. (tony) sample-corpus"``). Bare numbers
        ...
        if lead_page is None or not lead_residual:
            ...
            if len(lead_residual) < 3:
                ...
                # shared counts dicts (e.g. ``"a.w. sample-corpus"`` could
    ...
    # real residual text (``"a.w. (tony) sample-corpus"``). Internal matching
...
def _confirm_chrome_by_bbox(
    ...
    *,
    ...
    for span in text_spans:
        ...
        # (``"{N} A.W. sample-corpus"``) the same way it does trailing-digit
        # headers.
    ...
    for page_text in pages:
        ...
        for line in lines:
            ...
            if prefix and (prefix in chrome.headers or prefix in chrome.footers):
                ...
                # A.W. sample-corpus"``) — residual matches an even-page
