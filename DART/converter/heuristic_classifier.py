...
# naked ``<p>•    Item`` paragraphs — 323 of them on the sample-corpus textbook.
...
# ---------------------------------------------------------------------------
#
# sample-corpus audit: 39 ``<article role="doc-chapter">`` emissions vs 12
...
def _looks_like_activity_prompt(text: str) -> bool:
    ...
    Guards CHAPTER_OPENER promotion against sample-corpus-style activity
    ...
    * ``median_body_font_size`` — optional float. Callers can compute
    ...
    def __init__(
        ...
        *,
        ...
        # assembled HTML (see sample-corpus leading-digit footer regression).
    ...
    def _classify_one(self, block: RawBlock) -> ClassifiedBlock:
        ...
        if chapter_match and len(text) < 160:
            ...
            if not (
                ...
                # "Do you find..."). sample-corpus-style false positives
        ...
        # notes"). 149 short <p> bodies on sample-corpus match this shape;
        ...
        if _FOOTNOTE.match(text) and len(text) < 400:
            ...
            # ("{N} A.W. (Tony) sample-corpus") trips the FOOTNOTE regex
    ...
    def _matches_page_chrome(self, block: RawBlock, text: str) -> bool:
        ...
        footers ("{N} A.W. (Tony) sample-corpus") that survived the chrome-
    ...
    # per heading (14 per-block single-ol wrappers on sample-corpus). This
