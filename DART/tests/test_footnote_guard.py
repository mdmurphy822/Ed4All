...
Leading-digit running footers ("{N} A.W. (Tony) sample-corpus" on sample-corpus) trip the
...
class TestFootnoteGuardUsingPageNumberLines:
    def test_block_matching_page_chrome_superstring_not_footnote(self):
        # Simulated page-chrome record for sample-corpus: page 4's running
        # footer is "4 A.W. (Tony) sample-corpus".
        chrome = SimpleNamespace(
            headers=set(),
            footers={"a.w. (tony) sample-corpus"},
            page_number_lines={4: "4 A.W. (Tony) sample-corpus"},
        )
        clf = HeuristicClassifier(page_chrome=chrome)
        block = _make_block("4 A.W. (Tony) sample-corpus", page=4)
    ...
    def test_no_page_chrome_legacy_behavior_preserved(self):
        # Absent page_chrome → the guard is a no-op. The sample-corpus-style
    ...
    def test_block_with_no_page_skips_guard(self):
        ...
        clf = HeuristicClassifier(page_chrome=chrome)
        block = _make_block("4 A.W. (Tony) sample-corpus", page=None)
        result = clf.classify_sync([block])[0]
...
class TestFootnoteGuardUsingHeaderFooterSets:
    def test_block_matching_footer_set_without_page_mapping_guarded(self):
        ...
        clf = HeuristicClassifier(page_chrome=chrome)
        block = _make_block("7 A.W. (Tony) sample-corpus", page=7)
        result = clf.classify_sync([block])[0]
        assert result.role != BlockRole.FOOTNOTE
