...
# "{N} A.W. (Tony) sample-corpus" on the sample-corpus textbook) must be detected + stripped
...
class TestLeadingDigitsHelper:
    def test_strip_leading_digits_extracts_page_number(self):
        residual, page = _strip_leading_digits("164 a.w. (tony) sample-corpus")
        assert residual == "a.w. (tony) sample-corpus"
        assert page == 164
...
class TestLeadingDigitFooterDetection:
    def test_leading_digit_footer_detected_across_pages(self):
        # 10 even-page running footers "{N} A.W. (Tony) sample-corpus" — pre-
        ...
        # footer variants.
        stripped_joined = _FORM_FEED.join(chrome.stripped_pages)
        assert "2 A.W. (Tony) sample-corpus" not in stripped_joined
        assert "11 A.W. (Tony) sample-corpus" not in stripped_joined

    def test_mixed_odd_trailing_and_even_leading_chrome(self):
        # Odd-page trailing-digit header "Teaching in a Digital Age N";
        # even-page leading-digit footer "{N} A.W. (Tony) sample-corpus". Both
        ...
        assert "A.W. (Tony) sample-corpus" not in stripped_joined
