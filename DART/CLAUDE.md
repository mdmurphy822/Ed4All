...
* `MCP/tools/pipeline_tools.py::_raw_text_to_accessible_html`
  ...
  `/foo/sample-corpus.pdf` converted to `/foo/out/sample-corpus.html` persists
  figures under `/foo/out/sample_figures/` and the HTML carries
  relative `<img src="sample_figures/...png">` entries (the bundle is
...
Wave 17 inflation drops the sample-corpus HTML size from ~1.9 MB to ~1.5 MB.
...
**323 bullet-marker + 114 numbered-item paragraphs** on the sample-corpus
...
Pre-Wave-22, every DART Claude call site was uninstrumented — a sample-corpus
