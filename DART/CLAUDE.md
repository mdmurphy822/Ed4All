...
| `DART_CLAUDE_MODEL` | Pins the Anthropic model ID used by every DART converter surface (`DART/pdf_converter/cli.py::_resolve_default_model`, `claude_processor.py::_resolve_dart_claude_model`, `converter.py::PDFToAccessibleHTML.__init__`, `alt_text_generator.py::AltTextGenerator.__init__`). Resolution chain: explicit constructor / CLI arg > `DART_CLAUDE_MODEL` env var > `DART_CLAUDE_MODEL_DEFAULT` literal (`claude-sonnet-4-20250514`). Single source of truth lives in `DART/pdf_converter/claude_processor.py` (`DART_CLAUDE_MODEL_ENV` / `DART_CLAUDE_MODEL_DEFAULT` constants); the converter, alt-text generator, and CLI all import that helper so an operator retraining DART against a different teacher only sets one variable. (License: Anthropic Commercial Terms — DART HTML output may be ingested as Trainforge training chunks; see `docs/LICENSING.md` for the per-provider ToS posture.) When `DART_PROVIDER` resolves to a non-anthropic backend, this var pins the model identifier for that backend ONLY when set to a non-default value (per `_resolve_dart_model_for_provider`); an unset (or default-Claude) value falls through to the resolved provider's own model env (`LOCAL_SYNTHESIS_MODEL`, `TOGETHER_VISION_MODEL`, etc.) so a DART-on-local run picks up the local server's model identifier without surprises. |
| `DART_PROVIDER` | Wave W-D13. Selects the LLM backend for DART text-mode structure detection (`ClaudeProcessor`). Resolution chain (`DART/pdf_converter/claude_processor.py::_resolve_dart_provider`): explicit constructor / `--dart-provider` CLI arg > `DART_PROVIDER` env var > `"anthropic"` (legacy default). Values: `"anthropic"` (legacy `AnthropicBackend`), or any entry registered in `MCP/orchestrator/llm_backend.py::_OPENAI_COMPATIBLE_PROVIDERS` (`"local"`, `"together"`, `"together-vision"`, `"groq"`, `"fireworks"`, `"deepseek"`). Adding a new provider is a registry-entry change in that file — DART picks it up automatically (no per-provider subclass). **Recommended for license-clean DART**: `local` (Ollama / vLLM running an Apache 2.0 OSS model) since the rendered DART HTML is later ingested as Trainforge training chunks. (License: see `docs/LICENSING.md` for the per-provider ToS layer + per-model layer.) |
| `DART_VISION_PROVIDER` | Wave W-D13. Selects the LLM backend for DART vision-mode alt-text generation (`AltTextGenerator`). Resolution chain (`DART/pdf_converter/claude_processor.py::_resolve_dart_vision_provider`): explicit constructor / `--dart-vision-provider` CLI arg > `DART_VISION_PROVIDER` env var > `DART_PROVIDER` env var (fall-through) > `"anthropic"`. The split lets an operator pin a small text model for cheap structure detection AND a 90B+ vision model for alt-text without flipping every text call to the larger model. The constructor raises `ValueError` IMMEDIATELY when the resolved provider is not vision-capable — the "fail at startup, not mid-PDF" contract. Vision-capable opt-ins: `"anthropic"` (always), `"together-vision"` (Llama-3.2-90B-Vision via `TOGETHER_API_KEY`), or `"local"` with `LOCAL_VISION_CAPABLE=true` (or a `LOCAL_SYNTHESIS_MODEL` containing `vision` / `llava` / `-vl` substring per the resolver heuristic). Mirrors the `COURSEFORGE_OUTLINE_PROVIDER` / `COURSEFORGE_REWRITE_PROVIDER` split. (License: see `docs/LICENSING.md`.) |
| `LOCAL_VISION_CAPABLE` | Wave W-D13. When set to `1` / `true` / `yes` / `on` (case-insensitive), `MCP/orchestrator/llm_backend.py::resolve_openai_compatible_backend` flips the resolved `local` entry's `vision_capable` flag to `True` regardless of the registry default. Mirrors `LOCAL_SYNTHESIS_MODEL`'s opt-in posture: operators flip this on when they've loaded a vision-capable model into Ollama / vLLM (e.g. `llama3.2-vision`, `llava:13b`, `qwen2.5-vl:7b-instruct-q4_K_M`). Default unset → registry default `vision_capable: False` (text-only Qwen 14B). The resolver ALSO flips the flag heuristically when the resolved model identifier contains `vision` / `llava` / `-vl` substrings, so an operator who just sets `LOCAL_SYNTHESIS_MODEL` without remembering this env-var hatch still gets the right behaviour. |
| `TOGETHER_VISION_MODEL` | Wave W-D13. Overrides the default `meta-llama/Llama-3.2-90B-Vision-Instruct-Turbo` model for the `together-vision` registry entry. Common alternatives: smaller Llama-3.2-11B-Vision-Instruct-Turbo for cheaper inference. Reuses `TOGETHER_API_KEY` for auth (Together serves text + vision off one endpoint, but the registry split exists so an operator can pin DART vision at the vision model without flipping every text call). |

## SemantiK Opt-In Behavior Flags

SemantiK (the DART replacement — a license-clean semantic-cascade PDF→structured-content converter under `SemantiK/dart_semantic/` + `lib/semantik/`) owns the `SEMANTIK_*` env-var prefix. All toggles default to byte-stable / off-or-local behaviour to preserve backward compatibility; only the two model/provider selectors below (`SEMANTIK_SPECIALIST_PROVIDER` + the two `*_MODEL` seats) carry a `docs/LICENSING.md` row. Resolution everywhere is parse-with-fallback (unset / garbage / out-of-range → the documented default; a run never crashes on a malformed flag), mirroring the root `ED4ALL_*` posture.
...
`sampledigitalageaccessible` instead of
`sample_digital_age_accessible`, and the validator's valid
