...
Requires ``--course-dir`` pointing at the LibV2 course root (the
directory containing ``graph/``) flagged by an external review as
75% noise.
...
def main(argv: List[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to a LibV2 course root (the directory containing graph/).",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
