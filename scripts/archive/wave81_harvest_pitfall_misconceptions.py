...
historically harvested JSON-LD only, so for one archived corpus
(Path B regen) the per-chunk ``misconceptions[]`` count
...
This script applies the same bridging logic retroactively to an
already-archived corpus so the v1 baseline is
restored without re-running content generation. It:

* Opens the IMSCC zip under ``<archive>/source/imscc``.
...
def _resolve_archive(archive_arg: str) -> Path:
    p = Path(archive_arg)
