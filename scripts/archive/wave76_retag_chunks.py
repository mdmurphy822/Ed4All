...
Requires ``--course-slug <slug>`` to run against a LibV2 archive — the
same retag rules are content-addressed, so the script is generic.
...
def main(argv: List[str]) -> int:
    ...
    parser.add_argument(
        "--course-slug",
        required=True,
        help="LibV2 course slug to retag.",
    )
    parser.add_argument(
        "--libv2-root",
