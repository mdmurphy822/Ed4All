...
over-saturated: 7032 of 8324 edges (84%) on the RDF/SHACL calibration
corpus archive came from a temporal-cooccurrence cartesian. Wave 76 Worker D refined
...
Requires ``--archive`` to point at the LibV2 course directory being
re-pruned. ``--synthesized`` supplies the fallback
synthesized_objectives.json when ``<archive>/objectives.json`` is absent.

Usage::

    python scripts/archive/wave76_regen_pedagogy.py --archive LibV2/courses/<slug>
    python scripts/archive/wave76_regen_pedagogy.py --archive LibV2/courses/<slug> \\
...
def main(argv: Optional[list] = None) -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Course archive root (a LibV2 course directory).",
    )
    parser.add_argument(
        "--synthesized",
        type=Path,
