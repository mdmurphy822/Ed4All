...
| `ED4ALL_ANSWER_PRUNE_MIN_OVERLAP` | `0.25` | Float support threshold for the PRUNE decision (`citation_attribution.resolve_min_overlap`; also the `prune_min_overlap` kwarg). A citation supports a claim iff its 4-shingle containment ≥ this OR its content-token coverage ≥ 0.80 (fixed secondary floor). Deliberately low (precision of the keep-vs-drop decision over recall). The ADD bar is a higher, shingle-only constant (`ADD_MIN_SHINGLE=0.50`) + a relative "out-support the kept set" gate — not this knob. Out-of-range / garbage values fall back to the default. |
