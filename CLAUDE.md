...
| `trainforge_train` | `post_training_validation` | `eval_gating` | EvalGatingValidator (fails closed on regression / yes-bias / no-bias / source-match drop; also surfaces `EVAL_CONTENT_TYPE_ROLE_ALIGNMENT_LOW` warning-severity GateIssue when `content_type_role_alignment_summary.alignment_rate` < 0.70 — internal warning code, not a separately-registered gate) |
