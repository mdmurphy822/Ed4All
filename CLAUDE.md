...
| `ConceptCoverageAggregator` (W4.1) | `<libv2_course>/concept_coverage.json` (falls back to `<concept_graph>/concept_coverage.json`) — concept-keyed coverage table (the concept-graph analogue of `coverage_map`) surfacing which pedagogical surfaces (objectives, chunks, blocks, assessments) touch each concept. Only runs when `ED4ALL_CONCEPT_COVERAGE` is on (default OFF → no file). Emits `concept_coverage_aggregated`. | `schemas/aggregators/concept_coverage.schema.json` |
| `IntelligenceLevelAggregator` (W4.6) | `<libv2_course>/intelligence_level_report.json` (falls back to `<trainforge_dir>/...`) — deterministic (no-model) 0-5 capability rubric scoring a built course; the `faq` axis is intentionally always-absent (capability not built). Only runs when `ED4ALL_INTELLIGENCE_RUBRIC` is on (default OFF → no file). Emits `intelligence_level_scored`. | `schemas/aggregators/intelligence_level.schema.json` |
...
|----------|---------:|--------:|------:|
| `course_generation` | 17 | 40 | 57 |
| `rag_training` | 4 | 3 | 7 |
| `textbook_to_course` | 39 | 88 | 127 |
...
| `SEMANTIK_*` (DART replacement — SemantiK semantic-cascade converter; also honors the legacy `DART_THETA_DEVICE` compat env) | [`SemantiK/CLAUDE.md § Opt-In Behavior Flags`](SemantiK/CLAUDE.md) | 56 |
| `COURSEFORGE_*` / `COURSEPLANNER_*` / `TEXTBOOK_SYNTHESIS_*` | [`Courseforge/CLAUDE.md § Opt-In Behavior Flags`](Courseforge/CLAUDE.md) | 35 |
| `DECISION_*` / `ED4ALL_*` / `LOCAL_DISPATCHER_*` / `MCP_ORCHESTRATOR_*` / `LLM_*` (cross-cutting) | root (table below) | 143 |
...
| `ED4ALL_COURSE_IDENTITY_DEDUP` | unset (off) | W0.5 course-identity SPLIT-BRAIN guard (resolver `lib/course_identity.py::resolve_course_identity` / `course_identity_dedup_enabled`; wired in `lib/decision_capture.py::DecisionCapture._build_storage`). Default OFF → byte-identical legacy path: `DecisionCapture` constructs `LibV2Storage(course_code, auto_create=True)` exactly as before, so a capture handed the `normalize_course_code` hashed form (`"Ed4All"` → `"ED_472"`) still auto-creates an EMPTY skeleton at `courses/ed-472/` next to the populated verbatim-name course `courses/ed4all/` (the chunk-write path's dir). When truthy (`1`/`true`/`yes`/`on`), the capture routes through an idempotent resolver that produces ONE canonical slug (`lib/ontology/slugs.libv2_course_slug` of the verbatim name) + ONE `course_id`, GATES skeleton creation on whether a populated twin already resolves (no fresh empty hashed twin is minted — `auto_create` flips off when the canonical course is already populated), emits a `COURSE_IDENTITY_SPLIT_BRAIN` warning, and runs a ONE-SHOT cleanup of empty-skeleton twins. **Anti-fabrication:** the cleanup is gated STRICTLY on "the canonical course is itself populated on disk", and each twin is re-verified as a zero-regular-files skeleton immediately before `rmtree` — a legitimately-empty in-progress course (no populated canonical) is NEVER deleted. Best-effort: any resolver/cleanup failure logs a warning and falls back to the legacy construction so a capture is never lost. KNOWN LIMIT: only the recoverable direction is handled (the verbatim-name capture cleans the hashed empty twin); a capture handed ONLY the hash cannot recover the original name. Selects no provider/model → no `docs/LICENSING.md` row. Falsey / garbage → off (parse-with-fallback). |
...
| `ED4ALL_CHUNK_MERGE_FRAGMENT_FLOOR` | `0` (off) | W1b.4 runt-fragment merge floor (`resolve_merge_fragment_floor`, read once in `chunk_content` and threaded to `merge_small_sections(fragment_floor=...)`). Default `0` → byte-identical (only sub-floor runt results are folded; normal chunks untouched). A positive int (recommended operator value `5`) folds below-floor fragment chunks into a neighbor. Garbage / non-positive → `0`/off (parse-with-fallback). No gate, no decision_event. Selects no provider/model → no `docs/LICENSING.md` row. |
