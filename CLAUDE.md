...
| `ED4ALL_GATE_ADVISORY` | **Safety-critical.** When set to `1` / `true` / `yes` (case-insensitive), `Trainforge/training/runner.py:997` flips post-training eval gates from blocking to advisory — failures emit warning logs but do not abort the training run. Materially changes the promotion semantics described in `lib/governance/course_status.py`: with the flag on, a corpus that should have been classified `failed` (per a `eval_gating` / `family_completeness` GateIssue) can promote to `non_certified_archive` or beyond. Default unset → blocking. Use only for offline investigation runs where the operator wants the full eval-report surface without the abort. |
| `ED4ALL_LIBV2_ROOT` | Absolute path to the LibV2 root directory consumed by every LibV2-aware tool (`MCP/tools/dart_tools.py` archive resolution, `LibV2/tools/libv2/` importer, retrieval). Resolution chain: per-call kwarg > `ED4ALL_LIBV2_ROOT` > `<repo>/LibV2/` default. Test reference: `MCP/tests/test_libv2_root_resolution.py:14,62-66`. Set in operator deployments where LibV2 lives outside the repo tree (e.g. shared NFS mount). |
| `ED4ALL_MAILBOX_BASE_DIR` | Absolute path to the orchestrator's task-mailbox base directory consumed by `MCP/orchestrator/llm_backend.py:951` (Anthropic-mode dispatch) and the `task_mailbox.py` filesystem-IPC layer. Default unset → `<repo>/state/mailbox/`. Set in CI / test runs to redirect mailbox writes to `tmp_path`; production deployments may set it to a shared mount when the orchestrator runs across multiple processes. Test reference: `lib/tests/test_llm_backend.py:228,242`. |
| `ED4ALL_PRODUCTION` | When set to `1`, `MCP/server.py:94` enables production-mode FastMCP server settings (stricter validation, no debug endpoints, structured logging). Default `0` → development mode. Set in container deployments via the orchestrator runtime; not used by the CLI entry point. |
| `ED4ALL_ROOT` | Absolute path to the Ed4All project root, consumed by `lib/path_constants.py:75` (`PROJECT_ROOT_ENV_VAR` constant) and `lib/paths.py:27,127`. Resolution chain: `ED4ALL_ROOT` env var > `_find_repo_root()` walk-up from `__file__`. Set in container deployments where the repo lives at a non-standard mount point; default unset on developer machines (auto-detect works). |
| `ED4ALL_RUN_ID` | Per-run identifier consumed by every artifact emitter that needs a run-scoped tag (chunks, concept-graph, decision events). Resolution chain: explicit `RunManager(run_id=...)` > `ED4ALL_RUN_ID` env var > generated from `lib/run_manager.py` defaults. Tests use `conftest.py:48-72` to snapshot + restore the variable per-test for isolation. Production runs export the orchestrator-minted run ID into the environment so subprocess phase workers see the same value. |
| `ED4ALL_SKIP_ABLATION` | When set to `1` / `true` (case-insensitive), `Trainforge/training/runner.py:887-889` skips the post-training ablation pass. Default unset → ablation runs. Operator opt-out for fast-iteration training runs where eval-only signal is the goal. |
...
| `ED4ALL_STATE_RUNS_DIR` | Absolute path to the state-runs directory consumed by `lib/paths.py:91` (and downstream by checkpoint writers, GENERATION_PROGRESS.md updaters, decision-capture sidecar emitters). Default unset → `<repo>/state/runs/`. Honored by tests via `conftest.py:40,70` (`monkeypatch.setenv` in the per-test fixture isolates state writes to `tmp_path/state/runs/`). |
...
| `WAVE18_COS_PER_WEEK` | Wave 1.8 — objective-driven dynamic week count. Number of chapter objectives per week the planner targets when re-scaling `duration_weeks` post-objective-synthesis. Default `2` (calibrated against the rdf-shacl-551-2 archive: 30 COs / 15 weeks). Read once per `_plan_course_structure` invocation in `MCP/tools/pipeline_tools.py`. The re-scale formula is `max(8, ceil(len(chapter_objectives) / WAVE18_COS_PER_WEEK))` and only fires when `duration_weeks_explicit=False` and `mint_method != "user_supplied_objectives_json"`. Operator intent is preserved: passing `--weeks N` skips the re-scale entirely; reusing a hand-curated objectives JSON skips the re-scale (the operator's pacing decisions are encoded in the LO list itself). For short courses (≤16 COs at default `_COS_PER_WEEK=2`), the 8-week floor preserves the chapter-driven default. Override to `3` for slower-paced graduate courses or `1` for high-density vocational tracks. |
