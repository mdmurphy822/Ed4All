...
- **Review** in the recurring hygiene audit (re-run the file-hygiene +
  legacy-token scans); promote/close items as they change.
...
|---|------|----------|:---:|:------:|--------|---------|
| D1 | **Legacy-converter naming migration** — the retired AGPL PDF-conversion engine (superseded by SemantiK) left legacy naming tokens across tracked files. The write/emit paths were flipped to SemantiK vocabulary (`data-semantik-*` attrs, `semantik:` sourceIds, `semantik_structure` package, `semantik_chunks/` dirs) and the tracked-code + docs straggler purge has landed; the only retained legacy tokens are the allowlisted dual-READ compat shims (source-reference pattern, chunk-dir fallback, checkpoint/phase aliases, decision-event enums) that let un-migrated local corpora + old checkpoints still load. **REMAINING (owner-gated):** the corpus-body migration of the un-migrated local corpora + checkpoints (extend `migrate.py` with a corpus-body rewriter, re-sha), then the final tighten (retire the dual-read shims once no legacy corpora remain — incl. a trained table adapter's legacy cell-role attr, a retrain/post-process not-yet). | naming/migration | 🟠 | L | in-progress | `LibV2/tools/libv2/migrate.py` |
...
| D9 | **Stray local logs / `.bak` / `.contaminated`** — `runtime/state/logs/*.contaminated`, `runtime/state/workflows/*.pre-remediation.bak`, 3× 0-byte root `.log` (gitignored) | hygiene/scratch | 🟡 | S | open | `docs/file-audit-cleanup.md` |
...
| D19 | **`lib/validators` flat→subpackage deprecation-shim family awaiting deletion** — 9 PendingDeprecationWarning shims remain under `lib/validators/` (`training_pair_promotion`, `pair_claim_support`, `pair_lo_refs`, `pair_objective_delivery`, `libv2_manifest`, `libv2_model`, `libv2_packet_integrity`, `bloom_classifier_disagreement`, `bloom_structural_enforcement`), each re-exporting its `lib.validators.{pair,libv2,bloom}.*` subpackage module (W-D10 T10.1). The 2026-07-29 sanitization pass migrated every internal production importer to the new paths (`Trainforge/synthesize_training.py`, `LibV2/tools/libv2/importer.py`, `cli/commands/libv2_validate_packet.py`, `scripts/harness/calibrate_phase4_thresholds.py`) and deleted the zero-reference `bloom_alignment` shim; remaining old-path importers are test files that deliberately exercise the shims. Finish criteria (one pass, next minor version): delete the 9 shims + the flat-path back-compat aliases in `MCP/hardening/gate_input_routing.py` (~L3261/3274/3718/3789) + the shim-covering test imports. | naming/migration | 🟡 | S | open | shim module headers under `lib/validators/`; `MCP/hardening/gate_input_routing.py` |
| D20 | **Vendor-ingest drops image-only blocks — ~1,000 figure alts (11.5% of 8,788) never reach the output** — `lib/semantik/vendor_ingest.py:397` skips any non-table block whose `get_text()` is empty, so a standalone-figure block (image with alt, no prose) is silently dropped: 1,095 blocks corpus-wide on the introalgebra-bc corpus. Pre-existing class BEHIND the 2026-08-01 defect-3 fix (`3cd5f259` promoted ch6 `data-type="media"` `data-alt` spans, the worst instance, but did not change the empty-text skip). Fixing it adds ~1,095 blocks and materially shifts document structure + chunk boundaries, so it needs its own verification pass (interrogator re-run + chunk-count re-baseline), not a rider on a build. | conversion-quality | 🟠 | M | open | `lib/semantik/vendor_ingest.py:397`; interrogation report 2026-08-01 (runbook `plans/introalgebra-bc-02-run/RUNBOOK.md`) |
...
- **D1 (legacy-converter naming migration)** and **D2 (config sprawl)** are the big
  structural ones; both have written plans and should be scheduled as dedicated
  passes, not squeezed between features. D1 must be staged (dual-read shim → migrate
  persisted LibV2 corpora → flip emitters → tighten) — a big-bang rename breaks every
  existing course.
...
*Seeded 2026-07-11 from the file-hygiene audit, the legacy-token scan, and
