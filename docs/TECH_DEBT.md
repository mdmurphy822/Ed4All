...
| D13 | **Chapter-title derivation loses the title word** — with the page-glued running-header region dropped (D6 fix), the doc `<h1>` falls back to bare "Chapter 1" instead of "Chapter 1 Foundations"; the title word survives elsewhere in the outline but `build_chapters_ir` doesn't recover it | conversion-quality | 🟡 | M | open | `lib/semantik/cascade_ir.py::build_chapters_ir`; surfaced by D6 fix |
