...
2. Seed at least one answerable course so the ask flow returns real data,
   including at least one course whose citations resolve to **both** a DART HTML
   source page and an IMSCC member page (so the source-viewer route is exercised
...
| Source viewer | A source page opened from a citation (both DART and IMSCC sources). |
