...
|---|---:|---:|---:|---:|---:|
| COURSE_W (131 chunks, 28 LOs post-Worker-H) | 257 | 1451 | 0 | **0** | 1451 |
| COURSE_D (86 chunks, 26 LOs pre-Worker-H) | 28 | 172 | 0 | **142** | 30 |

Both courses produce zero `is-a` edges. COURSE_W produces zero `prerequisite` edges. COURSE_D produces many prerequisites but on a very different corpus shape. Details below.
...
|---|---:|---:|---:|---:|---:|
| COURSE_W (first 50 of 1451) | 50 | 19 | 31 | 0 | **100% non-noise** |
| COURSE_D (all 30) | 30 | 30 | 0 | 0 | **100% good** |
...
**Verdict**: **KEEP**. The rule is working as designed. The borderline tier on COURSE_W (31/50) is chunks connecting concepts via weaker co-occurrence (2–4 chunks) — not wrong, just lower-confidence. The existing `DEFAULT_THRESHOLD` (check `Trainforge/rag/inference_rules/related_from_cooccurrence.py`) is well-calibrated.

### `prerequisite` (LO-order heuristic) — **TUNE (not disable)**

| Course | Sampled | good | borderline | noise | Plausibility |
|---|---:|---:|---:|---:|---:|
| COURSE_W (total: 0) | 0 | — | — | — | **rule fires zero edges** |
| COURSE_D (first 50 of 142) | 50 | 14 | 21 | 15 | **30% noise** |

**COURSE_W fires ZERO prerequisite edges** despite Worker H's per-week LO specificity fix. Root cause: Worker H attaches all four Terminal Objectives (TO-01..TO-04) to every week's pages, PLUS the week-specific Chapter Objectives. Every chunk's `learning_outcome_refs` therefore starts with `to-01`, and since `to-01` is position 0 in `course.json::learning_outcomes`, the "earliest LO position" for every concept's first-seen chunk is **always 0**. The prerequisite rule's skew check produces no edges.

**COURSE_D fires 142 prerequisite edges** but with 30% noise. COURSE_D's LO-ref pattern is different: 74/86 chunks have NO `learning_outcome_refs` at all (rule skips them); the 12 chunks with refs use a sparse, inconsistent single-LO pattern that does produce position skew. The "good" and "borderline" edges are real pedagogy→pedagogy concept pairs (e.g. `addie→assessment`, `backward-design→blooms-taxonomy`). The 15 noise edges are all cross-domain — specifically, `accessibility→<pedagogy-concept>` edges where accessibility (Week 11 in COURSE_D) gets wrongly positioned as a prerequisite of earlier pedagogy concepts.

**Noise pattern** (COURSE_D examples):
...
1. **Exclude Terminal Objectives from position lookup.** Terminal objectives apply across the whole course by design; they don't drive "order". `_earliest_lo_position` should filter out LOs whose IDs start with `to-` (or whose `hierarchy_level == "terminal"` in course.json). This alone will fix COURSE_W's zero-edges problem.
...
3. **Optional: require source and target concepts to share at least one co-occurrence neighborhood** (i.e., both appear in chunks that share a third concept). This would filter the `accessibility→<pedagogy>` noise on COURSE_D cleanly.

These three changes together should move COURSE_D's noise rate from 30% to <10% and unlock meaningful edges on COURSE_W.
...
venv/bin/python -m Trainforge.process_course --imscc /path/to/COURSE_W.imscc --course-code COURSE_W --division STEM --domain computer-science --subdomain web-accessibility --output Trainforge/output/course_w --objectives /path/to/COURSE_W_objectives.json
venv/bin/python -m Trainforge.process_course --imscc /path/to/COURSE_D.imscc --course-code COURSE_D --division ARTS --domain education --subdomain instructional-design --output Trainforge/output/course_d --objectives /path/to/COURSE_D_objectives.json

# Inspect the semantic graph edge breakdown
python3 -c "
import json
from collections import Counter
for slug in ['course_w', 'course_d']:
