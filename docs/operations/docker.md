...
| Volume | Mount | Holds |
|--------|-------|-------|
| `ed4all-data` (named volume) | `/data` (= `ED4ALL_HOME`) | Mutable data: state, Courseforge exports, training captures, conversion (SemantiK) output (`semantik-output/`; a legacy `dart-output/` from a pre-task-#19 box is still dual-read), uploads, and the HF embedding cache (`/data/hf-cache`). |
