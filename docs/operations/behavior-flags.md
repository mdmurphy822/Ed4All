...
| `ED4ALL_IMSCC_MODULE_TITLES` | unset (off) | Uses terminal-objective text for IMSCC module titles. Unset uses the standard module-title formatter. Source: `Courseforge/scripts/packaging/package_multifile_imscc.py`. |
...
| `ED4ALL_RETRIEVAL_BLAS_THREADS` | `8` | BLAS thread-team cap scoped to the semantic search GEMV ONLY. Invalid or out-of-range values use the documented default. Source: `LibV2/tools/libv2/vector_index.py`, `docs/LICENSING.md`. |
| `ED4ALL_RETRIEVAL_TOPK_LEGACY` | unset (off) | Selects the full-sort top-k implementation instead of the partition-based implementation. Both preserve deterministic score and chunk-ID ordering. Source: `LibV2/tools/libv2/vector_index.py`. |
| `ED4ALL_WITH_ASSESSMENT_SFT` | unset (off) | Includes verified assessment-derived examples in synthesized SFT data. Truthy tokens (`1`, `true`, `yes`, `on`) enable it; false or unrecognized values leave it off. Source: `Trainforge/synthesize_training.py`, `Trainforge/generators/deterministic/assessment_sft_generator.py`. |
| `ED4ALL_WITH_GRAPH_SFT` | unset (off) | Includes graph-grounded examples in synthesized SFT data. Truthy tokens (`1`, `true`, `yes`, `on`) enable it; false or unrecognized values leave it off. Source: `Trainforge/synthesize_training.py`, `Trainforge/generators/deterministic/graph_sft_generator.py`. |
...
| `ED4ALL_ASSESSMENT_ITEM_BANK` | unset (off; auto-on for pipeline runs) | QTI 1.2 item-BANK sidecar. Source: `MCP/tools/pipeline_tools.py`, `Courseforge/scripts/packaging/qti_emitter.py`. |
