...
| `COURSEPLANNER_PROVIDER=local` | `nemotron-3-nano-30b-a3b` (via `LOCAL_SYNTHESIS_MODEL` / `COURSEPLANNER_MODEL`) | NVIDIA Open Model License | N/A (your hardware) | **Yes** | **Recommended for ToS-clean course planning** — closes the W-D11.F "course-outliner subagent" gap (W-D14) |
...
| `TRAINFORGE_ASSESSMENT_PROVIDER=local` | `nemotron-3-nano-30b-a3b` (via `LOCAL_SYNTHESIS_MODEL` / `TRAINFORGE_ASSESSMENT_MODEL`) | NVIDIA Open Model License | N/A (your hardware) | **Yes** | **Recommended for ToS-clean assessment generation** — closes the W-D11.F "assessment-generator subagent" gap (W-D15). Authored questions feed downstream `training_synthesis` so this surface IS training-data exposure. |
...
| `SEMANTIK_HEADING_JUDGE_MODEL` (seat via `SEMANTIK_HEADING_JUDGE_BASE_URL`, default `http://localhost:8123/v1`) | `nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4` on the local TRT-LLM Super seat | **NVIDIA Nemotron Open Model License** | N/A — self-hosted on DGX Spark-class hardware; a non-local endpoint carries that provider's ToS | **N/A — emits heading-level structure metadata for accessible HTML, not training-pair prose** | Preferred GLM-OCR enrichment judge. The default model and endpoint are local and training-clean under the self-hosted Nemotron posture. A distinct model or hosted endpoint requires a separate licensing review before its output is re-ingested into a training corpus. |
...
| Model | License | Role |
|-------|---------|------|
| `MoritzLaurer/DeBERTa-v3-base-mnli-fever-anli` | MIT (Microsoft DeBERTa-v3 base + MIT fine-tune) | NLI entailment scorer for prose/objective/grounding gates. Training-pair promotion calls this pinned zero-shot classifier directly, independent of `ED4ALL_BLOOM_TRIVOTE`. That flag governs only the separate `bloom_classifier_disagreement` gate, where the same process singleton is reused. |
| Six per-Bloom-level fine-tuned one-vs-rest heads (`lib/classifiers/bloom_deberta_heads.py::BloomDebertaHeads`; base `microsoft/deberta-v3-base`) | MIT (base checkpoint; the fine-tune is Ed4All-trained, not a third-party checkpoint) | **`ED4ALL_BLOOM_TRIVOTE_HEADS` voter-2 backend (default OFF — see `docs/operations/behavior-flags.md`).** Selected via `ED4ALL_BLOOM_HEADS_DIR` (default `models/bloom_classifiers`; no weights ship with the repo — a fresh checkout degrades to `None`/abstain, never a network fetch: `from_pretrained(<local path>)` only, `local_files_only=True`). Training data is pipeline-HARVESTED Bloom labels — `lib/bloom_labels/harvester.py` (behind `ED4ALL_HARVEST_BLOOM_LABELS`) walks already-emitted, already-licensed course artifacts (objective `bloom_level`, block `target_bloom`, assessment-item `bloom_level`) into `runtime/state/bloom_labels/labels.jsonl`; the trainer (`lib/classifiers/training/train_bloom_deberta.py`, operator-run, not wired into any pipeline phase) fine-tunes six one-vs-rest binary heads off that harvest against the pre-seeded local `microsoft/deberta-v3-base` snapshot. Inference-only quality gate like every other row in this table — no head output is distributed as course content or lands in `instruction_pairs.jsonl`, so this surface has zero training-data exposure regardless of the harvested-label source. |
...
| `cip29/bert-blooms-taxonomy-classifier` | **No license is documented for the pinned model. Do not bundle or redistribute its weights.** | Retained in the default flag-off compatibility registry, but the current ensemble dispatcher returns `unknown` without loading member weights. It is bypassed entirely when `ED4ALL_BLOOM_TRIVOTE` is enabled. The trivote path instead uses the generator's asserted level, the licensed DeBERTa zero-shot voter, and the deterministic verb ontology. |
| `distilbert-base-uncased-finetuned-sst-2-english` | Apache 2.0 | Retained as a legacy sentiment member in the default flag-off compatibility registry, but the current ensemble dispatcher does not load it. It is bypassed entirely when `ED4ALL_BLOOM_TRIVOTE` is enabled. |
