...
| `TRAINFORGE_SYNTHESIS_PROVIDER=anthropic` | `claude-sonnet-4-6` (via `ANTHROPIC_SYNTHESIS_MODEL`) | Anthropic proprietary | Anthropic Commercial Terms | **No** (without separate agreement) | Backward-compat default for `run_synthesis` when the env var pins `anthropic`; not recommended for training data |
| `TRAINFORGE_SYNTHESIS_PROVIDER=claude_session` | Active Claude Code session | Anthropic proprietary | Anthropic Consumer Terms (Pro/Max) | **No** | Even-more-restricted consumer-terms path; not recommended for training data |
