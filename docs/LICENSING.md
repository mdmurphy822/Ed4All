...
| `COURSEFORGE_PROVIDER=local` | `qwen2.5:14b-instruct-q4_K_M` (via `LOCAL_SYNTHESIS_MODEL`) | Apache 2.0 (Qwen2.5-7B/14B/32B default) | N/A (your hardware) | **Yes** | **Recommended for ToS-clean Courseforge content** |
...
| `COURSEPLANNER_PROVIDER=local` | `qwen2.5:14b-instruct-q4_K_M` (via `LOCAL_SYNTHESIS_MODEL` / `COURSEPLANNER_MODEL`) | Apache 2.0 (Qwen2.5-7B/14B/32B default) | N/A (your hardware) | **Yes** | **Recommended for ToS-clean course planning** — closes the W-D11.F "course-outliner subagent" gap (W-D14) |
...
| `TRAINFORGE_ASSESSMENT_PROVIDER=local` | `qwen2.5:14b-instruct-q4_K_M` (via `LOCAL_SYNTHESIS_MODEL` / `TRAINFORGE_ASSESSMENT_MODEL`) | Apache 2.0 (Qwen2.5-7B/14B/32B default) | N/A (your hardware) | **Yes** | **Recommended for ToS-clean assessment generation** — closes the W-D11.F "assessment-generator subagent" gap (W-D15). Authored questions feed downstream `training_synthesis` so this surface IS training-data exposure. |
| `TRAINFORGE_ASSESSMENT_PROVIDER=<registry>` | per the W-D12 `_OPENAI_COMPATIBLE_PROVIDERS` registry entry (`groq` / `fireworks` / `deepseek` / future additions) | per registry entry's underlying model license | per provider's ToS | per provider | Dynamic provider — adding a new entry to the registry surfaces here without a code edit per the W-D12 dynamic-references contract |
| `TRAINFORGE_SYNTHESIS_PROVIDER=anthropic` | `claude-sonnet-4-6` (via `ANTHROPIC_SYNTHESIS_MODEL`) | Anthropic proprietary | Anthropic Commercial Terms | **No** (without separate agreement) | **Gated opt-in** — `run_synthesis` fails closed with `SynthesisLicensingError` unless `TRAINFORGE_ALLOW_ANTHROPIC_SYNTHESIS=true` is set. Not recommended for training data; valid only with a separate Anthropic agreement permitting derivative training. |
| `TRAINFORGE_SYNTHESIS_PROVIDER=claude_session` | Active Claude Code session | Anthropic proprietary | Anthropic Consumer Terms (Pro/Max) | **No** | **Gated opt-in** — same `TRAINFORGE_ALLOW_ANTHROPIC_SYNTHESIS=true` acknowledgment required; even-more-restricted consumer-terms path. Not recommended for training data. |
...
| `TRAINFORGE_ALLOW_ANTHROPIC_SYNTHESIS=true` | n/a — acknowledgment gate | n/a | n/a | n/a | **Acknowledgment flag, not a provider selector.** Required to run training-pair synthesis through an Anthropic-family provider (`anthropic` / `claude_session`). Absent it, `Trainforge/synthesize_training.py::run_synthesis` raises `SynthesisLicensingError` before any LLM dispatch, pointing here. Set only if you hold a separate Anthropic agreement permitting derivative training. |
...
- **SemantiK conversion is license-clean by construction.** The PDF → accessible-HTML conversion stage (SemantiK, the replacement for the retired DART converter) carries **no PyMuPDF/MuPDF (AGPL-3) and no Poppler (GPL-2)** on its extraction path — that copyleft liability is removed. The subsystem ships Apache-2.0 (`SemantiK/LICENSE`) and runs fully offline by default (`SEMANTIK_SPECIALIST_PROVIDER=local`). The only ToS layer on the conversion path is the opt-in hosted-70B quality seat documented in the Synthesis providers table above, and even that produces published product content, not training data.
