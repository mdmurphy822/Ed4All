...
| H | `worker-h/courseforge-lo-specificity` | `worker-h` | done | Courseforge per-week learningObjectives specificity — fixes LO-fanout defect. | `Courseforge/scripts/rendering/generate_course.py`, `Courseforge/scripts/validation/validate_page_objectives.py` | A |
| I | `worker-i/packager-validation-gate` | `worker-i` | in review (PR #5) | Wire `validate_page_objectives.py` into `package_multifile_imscc.py` as a pre-package gate. | `Courseforge/scripts/packaging/package_multifile_imscc.py` | H |
