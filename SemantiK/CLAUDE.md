...
- **Local GGUF specialists (default).** `SEMANTIK_SPECIALIST_PROVIDER=local`
  ...
  default.
- **Hosted 70B endpoint seat (opt-in, never silent).** `SEMANTIK_SPECIALIST_PROVIDER=nvidia`
  (any non-local value) CONFIGURES an OpenAI-compatible hosted endpoint —
  defaults to the NVIDIA hosted 70B (`NVIDIA_API_KEY` / `NVIDIA_BASE_URL`).
  ...
  alone leaves the LOCAL specialists as the authoring tier; the 70B displaces
...
- **Batched two-phase Stage-6.** Phase 1 generates local drafts **batched by
  ...
  the 70B for a polish pass. Region-index keying is identical across modes, so
