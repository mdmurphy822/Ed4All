...
| `SEMANTIK_READING_ORDER_FIX` | unset (off) | Stage-5 **region reading-order re-sort** gate (`SemantiK/dart_semantic/structure_graph.py::resolve_reading_order_fix`). Default OFF → byte-identical to today: `build_structure_graph` appends regions in PASS order (Pass-2 all headings, Pass-3 tables/math, Pass-5 paragraphs, ...) and never re-sorts, so the flat Region list stays SEGREGATED by kind (every heading clustered at the front, every body paragraph at the back — the live OpenStax defect where EXAMPLE labels land at byte-4% and their bodies at byte-40%). Truthy (`1`/`true`/`yes`/`on`, case-insensitive) → a final **stable** sort of the region list by `min(feature_block_indices)` at the exit of `build_structure_graph` (after the coverage invariant, before return) restores document reading order; the just-verified coverage invariant guarantees disjoint FB sets, so the sort key is a total order with no ties → deterministic, and a permutation `assert` fails closed on a key bug. Falsey / unset / garbage → off (parse-with-fallback, mirrors `reviewer.resolve_structure_review_mode`). No provider/model — no `docs/LICENSING.md` row. |
