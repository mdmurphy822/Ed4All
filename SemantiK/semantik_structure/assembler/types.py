...
class DocCandidate:
    ...
    import with `semantik_structure.gates.types.GateResult` — the gates
    package depends on no other semantik_structure package; assembler depends
    on gates' types but only via this string annotation.
    """

    assembled: AssembledDoc
    source: str  # "fast-lane" | "offline-lane"
    gate_result: "GateResult"  # owned by semantik_structure.gates.types
