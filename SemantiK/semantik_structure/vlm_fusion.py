...
def resolve_keep_ordered_markers_mode() -> bool:
    ...
    markdown list marker — silently losing the exercise identifier. When this
    mode is ON the
...
def resolve_fusion_line_units_mode() -> bool:
    ...
    stem, "Solution", table, and practice lines into single mega-blocks.
    Reads
...
# poison when a single region carries thousands of repetitions of a short
# token cycle.
#
...
#     byte-identical chunks in a regression replay).
#
...
#
# Representative evidence lines it MUST catch (unrescued Tesseract-only OCR
# garbage that would otherwise ship verbatim):
...
# ---------------------------------------------------------------------------
# ®/© re-OCR duplicate-tail recognizer.
# ---------------------------------------------------------------------------
#
...
def _strip_markdown_structure(line: str) -> str:
    ...
    # Literal HTML entities: decode the common ones, drop
