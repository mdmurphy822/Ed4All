...
# RELATIVE imports (``.`` = the ``semantik_structure`` package) so the typed
# candidate classes bind to the SAME module object ``features`` /
# ``orchestrator`` build candidates from and ``cross_reranker`` routes
# against. The cascade is importable under both bare ``semantik_structure`` and
# package-prefixed ``SemantiK.semantik_structure``; an ABSOLUTE ``from
# semantik_structure.region_detection import ...`` here would bind the bare
# module even when the cascade was loaded as ``SemantiK.semantik_structure``,
...
def resolve_reading_order_fix() -> bool:
    ...
    **Shim over ``SEMANTIK_REGION_ORDER`` (ITEM5).** The min-FB reading-order
    ...
    :func:`semantik_structure.region_order.resolve_region_order_mode`; this shim
...
def _list_depth(state: Any, fb_index: int) -> int:
    ...
    Mapping mirrors
    :data:`semantik_structure.council.structure.LIST_NESTING_LABELS`:
...
def _resolve_cell_roles(
    ...
    and use :func:`~semantik_structure.council.table_cell_builder.build_cells`
...
def build_structure_graph(
    ...
    *,
    ...
    # inference time — see ``semantik_structure/council/structure.py``), so
