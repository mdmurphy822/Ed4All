...
class V2Config:
    ...
    def from_model_dir(cls, model_dir: Path | None = None) -> "V2Config":
        ...
        (``council/<name>/final`` via :func:`semantik_structure.paths.resolve_model`)
        and the Stage-6 Qwen specialists read ``qwen_specialists/config.yaml``
        — but :func:`semantik_structure.cascade.run_pipeline_v2` reads
        ...
        Path resolution goes through :func:`semantik_structure.paths.model_dir`
    ...
    def _scan_qwen_gguf_paths(model_root: Path) -> dict[str, Path]:
        ...
        :func:`semantik_structure.qwen_specialists.runtime._scan_adapter_artifacts`.
...
def resolve_local_v2_config(model_dir: Path | None = None) -> V2Config:
    ...
    :func:`semantik_structure.paths.model_dir` (honors ``SEMANTIK_MODEL_DIR`` /
    ``SEMANTIK_HOME``).
    """
