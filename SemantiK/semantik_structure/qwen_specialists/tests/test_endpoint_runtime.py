...
Run:
  ED4ALL_NLI_DEVICE=cpu ED4ALL_EMBEDDING_DEVICE=cpu \
    python -m pytest semantik_structure/qwen_specialists/tests/test_endpoint_runtime.py -q
"""
...
def test_transient_error_retried_then_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(
        "semantik_structure.qwen_specialists.endpoint_runtime._RETRY_BACKOFF_BASE_SECONDS",
        0.0,
    )
    rt = OpenAICompatibleRuntime(base_url="https://x/v1", api_key="K", model="m")
...
def test_cascade_runtime_mode_real_under_endpoint(monkeypatch):
    ...
    from semantik_structure.v2_config import V2Config
