...
``semantik_structure.pedagogical_units`` so BOTH the assembler absorb and the
Stage-5e regroup import ONE contract cycle-free. CPU-only, no model load.
"""

from __future__ import annotations

from semantik_structure.pedagogical_units import (
    ABSORB_MAX_RUN,
...
def test_no_import_cycle():
    ...
    importlib.import_module("semantik_structure.qwen_specialists.block_resegment")
    importlib.import_module("semantik_structure.assembler.gold_shell_markup")
    importlib.import_module("semantik_structure.assembler.shell")
