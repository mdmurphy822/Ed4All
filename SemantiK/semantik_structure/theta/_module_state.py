...
override ``SEMANTIK_SEMANTIC_MODEL_DIR`` still wins (highest precedence) but
...
for every input) is opt-in via ``SEMANTIK_ALLOW_THETA_STUB=1``. This
...
History note: this env var used to be ``SEMANTIK_REQUIRE_THETA_MODEL=1``
...
# CWD-relative ``models/...`` literal. ``SEMANTIK_SEMANTIC_MODEL_DIR`` still wins.
...
def _model_dir() -> Path:
    ...
    precedence (high → low):
      ``SEMANTIK_SEMANTIC_MODEL_DIR`` (legacy theta-specific override)
    ...
    """
    override = os.environ.get("SEMANTIK_SEMANTIC_MODEL_DIR", "").strip()
    if override:
...
def _stub_allowed() -> bool:
    ...
    Default is False — strict-by-default. Set ``SEMANTIK_ALLOW_THETA_STUB=1``
    to permit the 0.7 placeholder to flow into ``theta_score``.
    """
    return os.environ.get("SEMANTIK_ALLOW_THETA_STUB", "").strip() == "1"
...
def _get_model() -> "SemanticPreservationModel | None":
    ...
    fallback is not opted in (i.e. ``SEMANTIK_ALLOW_THETA_STUB!=1``).
    """
    key = "active"
    if key in _cached:
        return _cached[key]
    ...
    try:
        ...
        logger.warning(
            ...
            "(%r); falling back to stub_v1 because SEMANTIK_ALLOW_THETA_STUB=1 "
    ...
    if model is None and not stub_allowed:
        raise RuntimeError(
            ...
            f"override for dev/eval, set SEMANTIK_ALLOW_THETA_STUB=1."
        )

    if model is None:
        logger.warning(
            "Theta semantic-preservation model not found at %s; "
            "falling back to stub_v1 because SEMANTIK_ALLOW_THETA_STUB=1 "
            "is set.",
            model_dir,
        )
