"""Convert one PDF to an accessible HTML document and conformance audit.

This command wraps :func:`semantik_structure.cascade.run_full_cascade`. It can
write the assembled HTML product or run in report-only mode as the evaluator's
per-PDF process-isolation entry point. Failures remain nonzero and become
structured report errors when ``--report`` is supplied.

The command never substitutes missing model or validation dependencies. Its
reported WCAG value comes from the cascade's canonical
``wcag_status_under_mock`` result field.

Usage::

    # Full compatibility cascade:
    .venv/bin/python scripts/ops/pdf_to_html.py --pdf in.pdf --out out.html
...
# Surface missing conversion or validation dependencies as actionable CLI
# guidance before argument-dependent execution begins.
try:
...
def _enum_value(obj):
    """Render enum-backed cascade actions using their wire value."""
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        choices=("mock", "real"),
        default="real",
        help="real = configured local conversion models (default); "
        "mock = test-only model doubles.",
    )
    ap.add_argument(
        "--k",
        type=int,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.runtime == "real":
        print(
            "[pdf2html] real runtime: loading configured conversion models; "
            "streaming cascade progress below.",
            flush=True,
        )

    # Report-only mode omits the HTML payload while retaining result telemetry.
    report_only = args.out is None
    want_html = not report_only

    t0 = time.perf_counter()
    # A supplied validator owns the Chromium lifecycle for the complete run;
    # ``None`` is the command's explicit no-validation mode.
    ...
    try:
        ...
        # Report-mode callers receive the same failure in a parseable payload;
    ...
    if want_html:
        # A requested HTML product is mandatory; an absent payload is a
        # contract failure rather than an empty deliverable.
        ...
        # The product and its validation evidence are one output contract.
        # Audit-write failures propagate as a nonzero exit.
    ...
    # Read the cascade's canonical validation-result field.
