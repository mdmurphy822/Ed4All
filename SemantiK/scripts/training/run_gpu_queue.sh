...
#
#   nohup ./scripts/training/run_gpu_queue.sh > data/logs/gpu_queue.log 2>&1 &
#
# Order rationale (override with: ./scripts/training/run_gpu_queue.sh eval math gap):
...
for stage in "${ORDER[@]}"; do
  case "$stage" in
    ...
    gap)
      ...
      .venv/bin/python scripts/training/train_qwen_lora.py \
        --adapter gap_fill \
