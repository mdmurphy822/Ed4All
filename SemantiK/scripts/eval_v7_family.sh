#!/usr/bin/env bash
# Run a family of PDFs through scripts/make_pdf_vs_html.py, one process per
...
for pdf in "${PDFS[@]}"; do
  ...
  (
    "$PY" scripts/make_pdf_vs_html.py "$pdf" \
        --engine "$ENGINE" \
        --adapter "$ADAPTER" \
...
log "=== aggregating ==="
"$PY" scripts/_aggregate_v7_eval.py \
    --out "$RESULTS_JSON" \
