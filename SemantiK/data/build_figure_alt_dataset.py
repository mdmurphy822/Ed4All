...
``data._splits.stable_split_for_id``) so figures from one paper never straddle
...
from data.build_figure_catalog import EXTREME_ASPECT, TINY_MAX_PX
from scripts.fetch_ar5iv_figure_assets import _safe_path
...
def _resolve_image(rec: dict, mf_idx: dict[tuple[str, str, str], str]) -> Path | None:
    ...
    return _safe_path(src, doc_id, ref)
