...
Supersedes the ar5iv-only ``data/builders/build_table_qwen_data.py``: it adds OpenStax
...
Per-source parsing:
  ...
  * **pmc** (``raw_source_xml``, JATS) — normalized to HTML5 by
    ``data/sources/jats_tables.py`` (thead ``<td>``→``<th scope=col>``, caption hoist,
    graphic-only skip).
...
``data.common._splits.stable_split_for_id`` on a per-source stable doc id, so the same
...
Usage::
    ...
    python -m data.builders.build_table_qwen_multi --max-token-len 1024
    python -m data.builders.build_table_qwen_multi --sources pmc --max-pairs 50 \\
...
from data.builders.build_table_qwen_data import (
