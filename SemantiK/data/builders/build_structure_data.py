...
Source pipeline (mirrors data/builders/build_classifier_data_v2.py):
...
Usage:
    python data/builders/build_structure_data.py --workers 4
    python data/builders/build_structure_data.py --pair-dirs data/synthetic/forms
...
from data.alignment import structure_align
from data.augmentation.render_augment import (
    augment_html,
    resolve_render_augment_mode,
    resolve_render_profile_for,
)
from data.common.balance import add_cap_args, apply_caps_and_report
from data.alignment.structure_align import (
...
# Mirrors data/builders/build_classifier_data_v2.py TAG_TO_ROLE but maps to the
...
def _exact_render_extract_label(validator, output_html: str, pair_stem: str, stats: dict):
    ...
    Returns the :class:`~data.augmentation.render_capture.LabelResult` on success, or
    ``None`` with ``stats['error']`` set on render/extract failure."""
    from data.augmentation.render_capture import label_blocks_by_overlap, render_with_boxes
...
# ``data/alignment/structure_align.py`` and writes the additive ``structure_dataset/3``
...
def process_pair_global(validator, work: tuple) -> dict:
    ...
    ground truth via :func:`data.alignment.structure_align.align_blocks` and emit the
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        "lossless split/merge-aware aligner (data/alignment/structure_align.py) → "
    ...
    ap.add_argument(
        ...
        "data/builders/build_courtlistener_pseudo_structure.py) AFTER source-capping, so its rare "
    ...
    ap.add_argument(
        ...
        "/ dense, data/augmentation/render_augment.py), align back to the gold HTML, and emit "
    ...
    if args.include_legal_pseudo:
        ...
        if not legal_path.exists():
            raise SystemExit(
                ...
                "data/builders/build_courtlistener_pseudo_structure.py --build first."
            )
