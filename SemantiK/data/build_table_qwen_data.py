...
Splits use ``data._splits.stable_split_for_id`` so an arxiv_id that
...
Usage::
    ...
    python data/build_table_qwen_data.py                    # full build
    python data/build_table_qwen_data.py --max-pairs 30 \\
...
def _cell_role_from_classes(
    ...
    Mirror of ``data.build_table_specialist_data.assign_cell_label`` but
...
def _expand_grid(table_elem: etree._Element) -> list[list[dict[str, Any]]]:
    ...
    ``data/build_table_specialist_data.py``.
...
def main() -> None:
    ...
    coverage = {
        ...
        "notes": {
            "split_function": (
                "data._splits.stable_split_for_id (re-implementation of "
