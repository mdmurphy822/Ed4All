...
``import data.build_math_qwen_data`` stays GPU-import-free (see
``test_no_gpu_imports_in_builder``).

Usage::

    python data/build_math_qwen_data.py                  # full build
    python data/build_math_qwen_data.py --max-pairs 30 \\
...
def _load_filter_tokenizer(name: str) -> Any:
    ...
    ``import data.build_math_qwen_data`` stays free of any GPU /
