...
The single source of truth for what DART's conformance claim actually
...
``docs/wcag_coverage.md`` by ``scripts/gen_wcag_coverage_doc.py``.
...
* ``template-scope`` — owned by the DART page template (CSS/skip-UX),
...
COVERAGE: list[dict] = [
    ...
    {"sc": "1.4.3", "name": "Contrast (Minimum)", "level": "AA",
     ...
     "notes": "Document content sets no colors; the DART page template "
    ...
    {"sc": "2.4.7", "name": "Focus Visible", "level": "AA",
     "status": "template-scope", "enforced_by": [],
     "notes": "Focus styling is injected by the DART page template; "
    ...
    {"sc": "4.1.1", "name": "Parsing (REMOVED in WCAG 2.2)",
     ...
     "notes": "SC 4.1.1 was removed in WCAG 2.2; DART still hard-gates "
...
def render_markdown() -> str:
    ...
    ``scripts/gen_wcag_coverage_doc.py`` — edit COVERAGE, regenerate,
    never hand-edit the doc (a test enforces sync).
    """
    lines = [
        "# DART — WCAG 2.2 A/AA Coverage Map",
        "",
        "<!-- GENERATED by scripts/gen_wcag_coverage_doc.py from",
        "     dart_semantic/gates/wcag_coverage.py — do not hand-edit. -->",
        "",
        "Per-success-criterion statement of what DART's automated "
        ...
        "| `template-scope` | Owned by the DART page template, not document content |",
