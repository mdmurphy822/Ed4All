...
health check** (see :mod:`dart_semantic.theta.semantic_preservation`).
...
:mod:`dart_semantic.theta._module_state` for the policy. The stub-only
...
``theta/config.yaml`` (:func:`dart_semantic.theta.types.load_theta_config`,
...
invalid config raises :class:`~dart_semantic.theta.types.ThetaConfigError`
...
Stage 13 (:func:`dart_semantic.theta.exits.decide_exit`) reads the
...
from dart_semantic.assembler.types import AssembledDoc, GapKind
from dart_semantic.soft_reranker.document import (
    _score_heading_tree_balance,
    _score_ref_link_integrity,
)
from dart_semantic.soft_reranker.types import ScoredDoc
...
# ``dart_semantic.qwen_specialists.prompts.build_table_request``.
...
def evaluate(
    ...
    *,
    ...
    :func:`dart_semantic.theta.exits.decide_exit` overwrites it with the
    Stage-13 verdict.
    """
