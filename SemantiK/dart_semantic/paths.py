...
``dart_semantic`` package resolves at runtime. Mirrors Ed4All's
...
---------------------------------------------------------
    ...
    3. Package-relative default ``<SemantiK>/<dirname>`` — where
       ``<SemantiK>`` is the parent of the ``dart_semantic/`` package
...
llama-cpp / yaml. Importing ``dart_semantic.paths`` must stay cheap so a
...
# Package root = the ``SemantiK/`` directory (parent of the ``dart_semantic``
# package). ``parents[0]`` is ``dart_semantic/``; ``parents[1]`` is ``SemantiK/``.
