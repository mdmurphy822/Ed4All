...
You are running with `isolation: worktree`. A dedicated git worktree has been created for you off `dev-v0.2.0`. **Operate exclusively inside your worktree.** Do not `cd` out of it; do not use absolute paths that point at the user's main checkout at `/home/user/Projects/Ed4All`; do not touch `main`.
