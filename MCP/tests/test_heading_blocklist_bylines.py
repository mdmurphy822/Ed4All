...
- 2-token full names without a lead-in ("a textbook author", "John Smith")
- Single-initial names ("A.W. (Tony) sample-corpus")
...
class TestWave27LowSignalHeadings:
    ...
    def test_cover_design_byline_with_name(self):
        assert _cgh._is_low_signal_heading("Cover design by a textbook author") is True
    ...
    def test_two_token_full_name(self):
        assert _cgh._is_low_signal_heading("a textbook author") is True

    def test_edited_by_byline(self):
        assert _cgh._is_low_signal_heading(
            "Edited by a textbook author and Diego Calvanese"
        ) is True

    def test_initialed_name_with_parenthetical(self):
        assert _cgh._is_low_signal_heading("A.W. (Tony) sample-corpus") is True
