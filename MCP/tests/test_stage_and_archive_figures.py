...
def test_stage_dart_outputs_copies_figures_dir(tmp_path, monkeypatch):
    ...
    html_path, figures_dir = _write_dart_bundle(src_dir, "sample-corpus")
    ...
    result = asyncio.run(
        stage(
            ...
            course_name="sample-corpus",
        )
    )
    result_doc = json.loads(result)
    assert result_doc["success"] is True

    staged_html = cf_inputs / "run-1" / "sample-corpus.html"
    staged_figs = cf_inputs / "run-1" / "sample_figures"
