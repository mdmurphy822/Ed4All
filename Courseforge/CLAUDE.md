...
- GUI failure panel: the studio failure panel's `block_ids` and `pages` CSV options normalize into these same `target_block_instance_ids` / `target_page_ids` params (alongside `blocks` → `target_block_ids`) via `run_service._normalize_blocks_param`.
