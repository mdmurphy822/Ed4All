...
Courseforge-owned env-var toggles. Every `COURSEFORGE_*_*` flag is no-op when `COURSEFORGE_TWO_PASS` is unset. Long-form rationale also lives in `schemas/ONTOLOGY.md` § 12.
...
| `data-cf-curie` | block's outermost wrapper | Space-separated minted CURIE(s) force-injected by `RewriteProvider` when the rewrite LLM dropped a source block's CURIEs after the remediation budget. Carries minted (prose-corpus) CURIEs into the published HTML so the `rewrite_curie_anchoring` gate's str-path `extract_curies` scrape recognises them. Absent on RDF corpora whose rewrite output retains CURIEs in prose, and on any block the LLM authored cleanly. See § "Three-stage textbook synthesis" → dynamic CURIE minting. |
...
- **Rewrite tier** — `RewriteProvider` force-injects any still-missing CURIE as a `data-cf-curie` attribute on the block's outermost wrapper instead of raising on remediation-budget exhaustion, so minted CURIEs survive into the published HTML.
