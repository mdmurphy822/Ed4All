...
| `package_imscc` | Package course as IMSCC. Runtime delegates to `Courseforge/scripts/cartridge/package_multifile_imscc.py` (IMS CC v1.3 namespaces, per-week LO validation, `course_metadata.json` bundling). |
...
- `Courseforge/generators/outline/_outline_provider.py::OutlineProvider` — defaults: `local` provider, `qwen2.5:7b-instruct-q4_K_M` model, JSON-mode + lenient parse + grammar-aware backend payload (GBNF / JSON-Schema / vLLM guided / `format: json`). Both tiers now accept the full registry-superset provider set (the base's `_default_supported_providers()` — every `kind: openai_compatible` row in `config/endpoints.yaml` plus each tier's non-registry tags), so a `block_routing.yaml` row pinning e.g. `provider: groq` constructs cleanly instead of raising `ValueError` at the tier constructor.
- `Courseforge/generators/rewrite/_rewrite_provider.py::RewriteProvider` — defaults: `anthropic` provider, `claude-sonnet-4-6` model. Per-block-type pinning via `block_routing.yaml` (e.g. `assessment_item` always rewrite-tier Anthropic, `flip_card_grid` local); the registry-superset provider set above applies here too.
...
- `lib/validators/bloom/classifier_disagreement.py::BloomClassifierDisagreementValidator` (the old `lib/validators/bloom_classifier_disagreement.py` path is a back-compat re-export shim that emits a `PendingDeprecationWarning`) — warning-level Bloom-claim audit. No reliable Bloom classifier is currently provisioned: the legacy ensemble dispatch is unimplemented and abstains. `ED4ALL_BLOOM_TRIVOTE_HEADS` selects a staged, unproven MultiBERT/Ed4All-head path trained from synthesized corpus labels; it awaits validation and provisioned weights rather than representing a current classifier. The default `ED4ALL_BLOOM_TRIVOTE=off` path records abstention rather than a model vote or regeneration verdict. DeBERTa NLI remains active as a separate entailment/grounding heuristic; its availability must not be presented as a working Bloom classifier. See `docs/validation/validators.md` for current status and `docs/operations/behavior-flags.md` for retained configuration identifiers.
...
- `bert_ensemble_disagreement` / `bert_ensemble_dispersion_high` (legacy event identifiers retained for compatibility; the current unprovisioned path abstains).
