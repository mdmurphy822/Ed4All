...
_COURSEFORGE_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))


# Match the ``<h1>Week N Overview: {real title}</h1>`` tag emitted by
# :func:`Courseforge.scripts.rendering.generate_course.generate_week`. The real
...
# IMS CC 1.3 resource-type strings (Courseforge/docs/guides/troubleshooting.md:64-66).
...
def _validate_assessment_xsd(assessment: _Assessment) -> bool:
    ...
    if not xsd_name:
        return True
    xsd_path = _COURSEFORGE_ROOT / "schemas" / "imscc" / xsd_name
...
def _maybe_render_objectives_page(
    *,
    ...
    try:
        from Courseforge.scripts.rendering.render_learning_objectives_page import (
...
def validate_content_objectives(
    ...
    """
    from Courseforge.scripts.validation.validate_page_objectives import (
        discover_html_pages,
