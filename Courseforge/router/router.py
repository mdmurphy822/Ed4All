...
# ---------------------------------------------------------------------------
# Best-of-N NLI-verifier selection (W5 §3 — the keystone)
# ---------------------------------------------------------------------------
#
# The self-consistency loops below already sample N candidates and run the
# validator chain per candidate; today they early-return on the FIRST candidate
# that PASSES the chain (a binary gate-pass selector). W5 flips the selector to
# ``argmax(entailment_rate)`` AMONG candidates that pass the validator chain
# (the validator-pass requirement is preserved — a failing candidate is never
# selectable), cover the objective, and carry zero contradictions. This turns
# the W4 provenance gate into a generation-time selector, not just a post-hoc
# reject. The flip is strictly opt-in: it fires only when
...
class _CandidateVerdict:
    """NLI-verifier verdict for one best-of-N candidate (W5 §3.1)."""
...
def score_candidate(
    ...
    *,
    ...
    """Score one best-of-N candidate with the W4 NLI verifier (W5 §3.1).

    A thin wrapper around ``score_groundedness`` (the W4 block-prose entailment
    verifier — the NLI model is already loaded for the gates, so best-of-N adds
    only N× forward passes over the extra candidates, not a second model) plus a
    cheap objective-coverage signal. Returns a :class:`_CandidateVerdict`:

    * ``entailment_rate`` — ``score_groundedness(...).groundedness_rate`` over the
      candidate's resolved cited chunk passages (0.0 when nothing scorable).
    * ``contradicted`` — any contradicted claim (a contradiction is fabrication
      that *conflicts* with source — fatal to selection).
    * ``covers_objective`` — whether the candidate references ≥1 objective id
      (the cheap structural coverage signal; the selector requires this so a
      high-entailment but pedagogically-thin block can't win — W5 risk #1).
    * ``delivery_score`` — the entailment_rate reused as the tie-break weight
      (objective-covering candidates tie-break on grounding strength).

    Graceful degrade: ``nli`` resolves to ``None`` ⇒ ``entailment_rate=0.0`` +
    ...
    passages = _candidate_cited_passages(candidate, source_chunks)
    ...
    covers_objective = _candidate_covers_objective(candidate, objectives)
    return _CandidateVerdict(
        entailment_rate=entailment_rate,
        contradicted=contradicted,
        covers_objective=covers_objective,
        delivery_score=entailment_rate,
    )


def _candidate_prose_text(candidate: "Block") -> str:
    """Strip the candidate Block's content to plain text for NLI scoring."""
    ...
    if not raw:
        return ""
    try:
        from lib.utils.html_text import strip_html_to_text  # noqa: PLC0415
...
def _candidate_cited_passages(
    candidate: "Block", source_chunks: Optional[List[Any]]
) -> List[Dict[str, str]]:
    """Resolve the candidate's cited chunk ids → ``[{chunk_id, text}]`` premises.

    ``source_chunks`` is the per-block chunk set threaded through ``route``; it may
    be a list of dicts (``{id|sourceId, text}``) or a mapping. We index it by id
    and resolve the candidate's ``source_references[].sourceId`` + ``source_ids``.
    """
    by_id: Dict[str, str] = {}
    for chunk in source_chunks or ():
        if isinstance(chunk, dict):
            cid = chunk.get("id") or chunk.get("sourceId") or chunk.get("chunk_id")
            txt = chunk.get("text") or chunk.get("body")
            if isinstance(cid, str) and isinstance(txt, str):
                by_id[cid] = txt
        else:
            cid = getattr(chunk, "id", None) or getattr(chunk, "sourceId", None)
    ...
    cited_ids: List[str] = []
    seen: set = set()
    for ref in getattr(candidate, "source_references", ()) or ():
    ...
    for sid in getattr(candidate, "source_ids", ()) or ():
        if isinstance(sid, str) and sid and sid not in seen:
            seen.add(sid)
            cited_ids.append(sid)
    ...
    for cid in cited_ids:
        ...
        if isinstance(txt, str) and txt.strip():
            passages.append({"chunk_id": cid, "text": txt})
    return passages


def _candidate_covers_objective(
    candidate: "Block", objectives: Optional[List[Any]]
) -> bool:
    """True when the candidate references ≥1 objective id (cheap coverage signal).

    Reads the candidate's ``objective_ids`` / ``objective_refs`` surfaces; when no
    objective list is supplied at all we treat coverage as satisfied (the
    selector then falls back to pure grounding argmax — the objective gate is only
    meaningful when objectives are threaded).
    """
    if not objectives:
        return True
    declared: set = set()
    for attr in ("objective_ids", "objective_refs"):
        for oid in getattr(candidate, attr, ()) or ():
            if isinstance(oid, str) and oid:
                declared.add(oid.upper())
    content = getattr(candidate, "content", None)
    if isinstance(content, dict):
        for oid in content.get("objective_refs", []) or content.get("objective_ids", []) or []:
            if isinstance(oid, str) and oid:
                declared.add(oid.upper())
    if not declared:
        return False
    ...
    for obj in objectives:
        ...
        if isinstance(oid, str) and oid:
            objective_ids.add(oid.upper())
    if not objective_ids:
        # No resolvable objective ids to check against — coverage is unverifiable,
        # so don't penalise the candidate.
        return True
    return bool(declared & objective_ids)


def _select_best_of_n_winner(
    scored: List[Tuple[int, "Block", _CandidateVerdict]],
) -> Tuple[Optional[int], Optional["Block"], bool]:
    """Argmax over PASSING candidates by (entailment_rate, delivery_score).

    ``scored`` is the list of ``(index, candidate, verdict)`` for candidates that
    ALREADY PASSED the validator chain (the validator-pass requirement is enforced
    by the caller — a failing candidate never reaches this helper). Selection:

    1. Prefer candidates that ``covers_objective`` AND are ``not contradicted``;
       among them pick ``argmax((entailment_rate, delivery_score))``.
    2. Fallback (no clean candidate): keep the highest ``entailment_rate`` overall
       and signal ``no_clean=True`` so the caller stamps an escalation marker —
       never fabricate a winner.

    Returns ``(winning_index, winning_candidate, no_clean)``. ``(None, None, ...)``
    only when ``scored`` is empty.
    """
    if not scored:
        return None, None, False
    clean = [
        (i, cand, v)
        for (i, cand, v) in scored
        if v.covers_objective and not v.contradicted
    ]
    if clean:
        best = max(clean, key=lambda t: (t[2].entailment_rate, t[2].delivery_score))
        return best[0], best[1], False
    # No clean candidate: highest entailment overall, flagged.
    best = max(scored, key=lambda t: t[2].entailment_rate)
...
class CourseforgeRouter:
    ...
    def __init__(
        ...
        *,
        ...
        # W5 best-of-N: injectable NLI verifier for the entailment-argmax
        # selector. Default None ⇒ lazy-resolved from the singleton on first
        # use (only when the entailment_argmax selector is active). Tests pass a
        # deterministic fake.
        nli: Optional[Any] = None,
    ) -> None:
        ...
        # W5 best-of-N NLI verifier (injectable). ``_nli_resolved`` caches the
        # singleton resolution so we don't re-probe the ~750MB model per block.
        self._nli = nli
        self._nli_resolved = nli is not None  # True ⇒ ``self._nli`` is final
    ...
    def route_with_self_consistency(
        ...
        **overrides: Any,
        ...
        # W5 best-of-N: when the entailment-argmax selector is active (C5 mode
        # projects COURSEFORGE_BEST_OF_N_SELECT_BY=entailment_argmax), the loop
        # COLLECTS every PASSING candidate + its NLI verdict instead of
        # early-returning on the first pass, then selects argmax(entailment_rate)
        # post-loop. ``gate_pass`` (default) keeps the legacy first-passing
        # behaviour byte-for-byte. ``best_of_n_scored`` holds
        # (index, candidate, verdict) for passing candidates only — the
        # validator-pass requirement is preserved (a failing candidate never
        # reaches the scorer).
        ...
        for i in range(resolved_n):
            ...
            if all_passed and best_of_n_mode:
                ...
                # and COLLECT it (do not early-return). The post-loop selector
                # picks argmax(entailment_rate) among objective-covering,
                # non-contradicted passing candidates. The validator-pass gate
                # is already satisfied here — only passing candidates are scored.
                ...
                # Keep sampling the remaining budget for a stronger candidate.
                continue
            if all_passed:
        ...
        # W5 best-of-N: post-loop argmax selection over the PASSING candidates
        # collected above (entailment_argmax mode only). Picks the strongest
        # objective-covering, non-contradicted candidate; if none is clean, keeps
        # the highest-entailment one and stamps an escalation marker (never
        # fabricates a winner). When no candidate passed validation at all,
        # ``best_of_n_scored`` is empty and we fall through to the legacy
        # escalation/all-fail resolution below unchanged.
        if best_of_n_mode and best_of_n_scored and winner is None:
            sel_index, sel_candidate, no_clean = _select_best_of_n_winner(
                best_of_n_scored
            )
            if sel_candidate is not None:
                ...
                touch = Touch(
                    ...
                    timestamp=timestamp,
                    decision_capture_id=self._build_router_capture_id(
                        sel_candidate, f"best_of_n_winner_{sel_index}"
                    ),
                    purpose="self_consistency_winner",
                )
                sel_candidate = sel_candidate.with_touch(touch)
                if no_clean:
                    sel_candidate = dataclasses.replace(
                        sel_candidate,
                        escalation_marker="best_of_n_no_clean_candidate",
                    )
                winner = sel_candidate
                winning_index = sel_index
    ...
    def route_rewrite_with_remediation(
        ...
        **overrides: Any,
        ...
        # W5 best-of-N (rewrite tier): identical seam to the outline loop —
        # collect PASSING candidates + NLI verdicts under entailment_argmax mode,
        # select argmax post-loop. ``gate_pass`` (default) keeps the legacy
        # first-passing behaviour.
        select_by = self._best_of_n_select_by()
        best_of_n_mode = select_by == "entailment_argmax"
        best_of_n_scored: List[Tuple[int, Block, _CandidateVerdict]] = []
        ...
        for i in range(resolved_n):
            ...
            last_candidate = candidate
            if all_passed and best_of_n_mode:
                # W5 best-of-N (rewrite): score + collect the PASSING candidate,
                # keep sampling. Post-loop selector picks argmax(entailment_rate).
                verdict = score_candidate(
                    candidate,
                    source_chunks,
                    objectives,
        ...
        # W5 best-of-N (rewrite): post-loop argmax over collected passing
        # candidates (entailment_argmax mode). Mirrors the outline tier.
        if best_of_n_mode and best_of_n_scored and winner is None:
            sel_index, sel_candidate, no_clean = _select_best_of_n_winner(
                best_of_n_scored
            )
            if sel_candidate is not None:
                ...
                touch = Touch(
                    ...
                    timestamp=timestamp,
                    decision_capture_id=self._build_router_capture_id(
                        sel_candidate, f"best_of_n_rewrite_winner_{sel_index}"
                    ),
                    purpose="self_consistency_winner",
                )
                sel_candidate = sel_candidate.with_touch(touch)
    ...
    def _best_of_n_select_by(self) -> str:
        """Resolve the active best-of-N selector (``gate_pass`` default).

        Reads ``COURSEFORGE_BEST_OF_N_SELECT_BY`` via the technique-mode helper.
        Returns ``entailment_argmax`` only when explicitly projected (by the C5
        mode); any other / unset value yields ``gate_pass`` so the legacy
        first-passing-candidate selector runs unchanged (additive).
        """
        try:
            from lib.generation.technique_modes import resolve_select_by  # noqa: PLC0415
    ...
    def _resolve_best_of_n_nli(self) -> Optional[Any]:
        ...
        Injected instance wins; otherwise the process-singleton
        ``NliClassifier.get_or_load()`` (returns ``None`` on missing extras —
        graceful degrade, the selector then falls back to first-passing). Cached
        so the ~750MB model is probed once per router, not per block.
        """
        if self._nli_resolved:
            return self._nli
        self._nli_resolved = True
        try:
            from lib.classifiers.nli_classifier import NliClassifier  # noqa: PLC0415

            self._nli = NliClassifier.get_or_load()
        except Exception:  # noqa: BLE001
            self._nli = None
        return self._nli

    def _emit_best_of_n_decision(
        self,
        *,
        block: Block,
        tier: str,
        ...
        """Emit the ``block_best_of_n_selection`` decision (W5 §3.3, replayable)."""
        if self._capture is None:
            return
        rates = [round(v.entailment_rate, 4) for v in verdicts]
        spread_min = min(rates) if rates else 0.0
        spread_max = max(rates) if rates else 0.0
        chosen = verdicts[winning_index] if winning_index is not None and 0 <= winning_index < len(verdicts) else None
        best_rate = round(chosen.entailment_rate, 4) if chosen else 0.0
        delivery = round(chosen.delivery_score, 4) if chosen else 0.0
        contradicted = chosen.contradicted if chosen else None
        temp = getattr(spec, "temperature", None)
        model = getattr(spec, "model", None)
        try:
            self._capture.log_decision(
                decision_type="block_best_of_n_selection",
                decision=(
                    f"selected candidate {winning_index}/{n} for "
                    f"{getattr(block, 'block_id', block.block_type)}"
                ),
                rationale=(
                    f"entailment_rate spread {spread_min:.2f}..{spread_max:.2f} "
                    f"across N={n}; chose idx {winning_index} (rate={best_rate:.2f}, "
                    f"delivery={delivery:.2f}, contradicted={contradicted}); "
                    f"tier={tier}, temp={temp}, model={model}, "
                    f"no_clean={no_clean}, select_by={select_by}"
                ),
                ml_features={
                    "n": n,
                    "winning_index": winning_index,
                    "per_candidate_entailment_rates": rates,
                    "select_by": select_by,
                    "tier": tier,
                    "no_clean_candidate": no_clean,
                },
            )
